...
def _save(im, fp, filename, check=0):
    ...
    if im.mode == "P":
        ...
        # the lower bpp bits are significant.  We invert them to match the
        # Palm.
        bpp = im.info["bpp"]
    ...
    if colormapsize > 0:
        ...
        for i in range(256):
            ...
            if colormapmode == 'RGB':
                fp.write(
                    o8(colormap[3 * i]) + o8(colormap[3 * i + 1]) + o8(colormap[3 * i + 2]))
            elif colormapmode == 'RGBA':
                fp.write(
                    o8(colormap[4 * i]) + o8(colormap[4 * i + 1]) + o8(colormap[4 * i + 2]))
