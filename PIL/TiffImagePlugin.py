...
class ImageFileDirectory(collections.MutableMapping):
    ...
    def load(self, fp):
        ...
        for i in range(i16(fp.read(2))):
            ...
            if len(data) != size:
                warnings.warn(
                    "Possibly corrupt EXIF data.  Expecting to read %d bytes but only got %d. Skipping tag %s" %
                    (size, len(data), tag))
                continue
...
# little endian is default except for image modes with explict big endian
# byte-order

SAVE_INFO = {
    # mode => rawmode, byteorder, photometrics, sampleformat, bitspersample,
    # extra
    "1": ("1", II, 1, 1, (1,), None),
    "L": ("L", II, 1, 1, (8,), None),
    "LA": ("LA", II, 1, 1, (8, 8), 2),
...
def _save(im, fp, filename):
    ...
    ifd[COMPRESSION] = COMPRESSION_INFO_REV.get(
        compression,
        ...
        for k, v in itertools.chain(
            ...
            if k not in atts and k not in blocklist:
                if isinstance(v[0], tuple) and len(v) > 1:
                    ...
                    # flatten to floats, following
                    # tiffcp.c->cpTag->TIFF_RATIONAL
