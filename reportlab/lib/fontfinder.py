...
class FontDescriptor:
    ...
    def __init__(self):
        ...
        # true if it's italic or oblique or somehow slanty
        self.isItalic = False
        ...
        # defined only for type='type1pc', or 'type1mac'
...
class FontFinder:
    ...
    def getFamilyNames(self):
        ...
        names = sorted(self._fontsByFamily.keys())
        return names
    ...
    def getFont(self, familyName, bold=False, italic=False):
        ...
        raise KeyError(
            "Cannot find font %s with bold=%s, italic=%s" %
    ...
    def search(self):
        ...
        if self.useCache:
            ...
            if rl_isfile(cfn):
                try:
                    self.load(cfn)
                    # print "loaded cached file with %d fonts (%s)" %
                    # (len(self._fonts), cfn)
                    return
                except:
                    # pickle load failed.  Ho hum, maybe it's an old pickle.
                    # Better rebuild it.
                    pass
