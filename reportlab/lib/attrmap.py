...
# history
# http://www.reportlab.co.uk/cgi-bin/viewcvs.cgi/public/reportlab/trunk/reportlab/lib/attrmap.py
__version__ = ''' $Id$ '''
            ...
            if isinstance(BASE, AttrMap):
                ...
                for B in BASE:
                    ...
                    if am is not self:
                        ...
                        raise ValueError(
                            'BASE=%s has wrong kind of value' %
                            ascii(B))
...
def hook__setattr__(obj):
    if not hasattr(obj, '__attrproxy__'):
        ...
        obj.__class__ = new.classobj(
            C.__name__, (C,) + C.__bases__, {
                '__attrproxy__': [], '__setattr__': lambda self, k, v, osa=getattr(
                    obj, '__setattr__', None), hook=hook: hook(
                    self, k, v, osa)})
