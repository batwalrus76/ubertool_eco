...
# history
# http://www.reportlab.co.uk/cgi-bin/viewcvs.cgi/public/reportlab/trunk/reportlab/lib/colors.py
__version__ = ''' $Id$ '''
...
class Color:
    ...
    def __repr__(self):
        return "Color(%s)" % fp_str(
            *(self.red, self.green, self.blue, self.alpha)).replace(' ', ',')
...
class CMYKColor(Color):
    ...
    def __init__(self, cyan=0, magenta=0, yellow=0, black=0,
        ...
        self.red, self.green, self.blue = cmyk2rgb(
            (cyan, magenta, yellow, black))

        if density < 1:
            # density adjustment of rgb approximants, effectively mix with
            # white
...
class PCMYKColor(CMYKColor):
    ...
    def __repr__(self):
        return "%s(%s%s%s%s%s)" % (self.__class__.__name__,
                                   fp_str(self.cyan * 100,
                                          self.magenta * 100,
                                          self.yellow * 100,
                                          self.black * 100).replace(' ',
                                                                    ','),
                                   (self.spotName and (',spotName=' + repr(self.spotName)) or ''),
...
def linearlyInterpolatedColor(c0, c1, x0, x1, x):
    ...
    if x < x0 - 1e-8 or x > x1 + 1e-8:  # fudge factor for numerical problems
        raise ValueError(
            "Can't interpolate: x=%f is not between %f and %f!" %
            (x, x0, x1))
    if x <= x0:
        return c0
    elif x >= x1:
...
# this is a hangover from PIDDLE - suggest we ditch it since it is not
# used anywhere
...
def hsl2rgb(h, s, l):
    if l <= 0.5:
        ...
        m2 = l + s - l * s
    m1 = l * 2 - m2
    return hue2rgb(m1, m2, h +
                   1. /
                   3), hue2rgb(m1, m2, h), hue2rgb(m1, m2, h -
                                                   1. /
                                                   3)
...
class cssParse:
    ...
    def pcVal(self, v):
        ...
        try:
            ...
            raise ValueError(
                'bad percentage argument value %r in css color %r' %
                (v, self.s))
        return c
    ...
    def rgbVal(self, v):
        ...
        try:
            ...
            raise ValueError(
                'bad argument value %r in css color %r' %
                (v, self.s))
    ...
    def hueVal(self, v):
        ...
        try:
            ...
            raise ValueError(
                'bad hue argument value %r in css color %r' %
                (v, self.s))
    ...
    def alphaVal(self, v, c=1, n='alpha'):
        try:
            ...
            raise ValueError(
                'bad %s argument value %r in css color %r' %
                (n, v, self.s))

    _n_c = dict(
        pcmyk=(
            4, 100, True, False), cmyk=(
            4, 1, True, False), hsl=(
                3, 1, False, True), rgb=(
                    3, 1, False, False))
    ...
    def __call__(self, s):
        ...
        if cmyk:
            ...
            if hsl:
                ...
                R, G, B = list(
                    map('%' in n[0] and self.rgbPcVal or self.rgbVal, n))
...
class toColor:
    ...
    def __init__(self):
        # used for overriding/adding to existing color names
        self.extraColorsNS = {}
...
def _enforceSEP_BLACK(c):
    ...
    if not isinstance(tc, CMYKColorSep):
        if isinstance(
                tc,
                Color) and tc.red == tc.blue == tc.green:  # ahahahah it's a grey
...
def _chooseEnforceColorSpace(enforceColorSpace):
    if enforceColorSpace is not None and not isinstance(
            enforceColorSpace,
