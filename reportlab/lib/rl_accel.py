...
if 'instanceStringWidthT1' in _py_funcs:
    if isPy3:
        def instanceStringWidthT1(self, text, size, encoding='utf8'):
            ...
            return sum([sum(map(f.widths.__getitem__, t)) for f, t in unicode2T1(
                text, [self] + self.substitutionFonts)]) * 0.001 * size
    else:
        def instanceStringWidthT1(self, text, size, encoding='utf8'):
            """This is the "purist" approach to width"""
            if not isUnicode(text):
                text = text.decode(encoding)
            return sum([sum(map(f.widths.__getitem__, list(map(ord, t)))) for f, t in unicode2T1(
                text, [self] + self.substitutionFonts)]) * 0.001 * size
...
if 'asciiBase85Encode' in _py_funcs:
    def asciiBase85Encode(input):
        ...
        if remainder_size > 0:
            ...
            lastword = chr(c1 + 33) + chr(c2 + 33) + \
                chr(c3 + 33) + chr(c4 + 33) + chr(c5 + 33)
...
if 'asciiBase85Decode' in _py_funcs:
    def asciiBase85Decode(input):
        ...
        # check end
        assert stripped[-2:
                        ] == '~>', 'Invalid terminator for Ascii Base 85 Stream'
        ...
        for i in range(whole_word_count):
            ...
            assert num == 16777216 * b1 + 65536 * \
                b2 + 256 * b3 + b4, 'dodgy code!'
            out(chr(b1))
            out(chr(b2))
            out(chr(b3))
            out(chr(b4))
        ...
        for cmd in (
            ...
            s = ';'.join(
                ("from reportlab.pdfbase.pdfmetrics import getFont",
                 "from %s import unicode2T1,_instanceStringWidthU" %
                 modname,
