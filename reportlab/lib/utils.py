...
def isFunction(v):
    return isinstance(v, type(isFunction))
...
def isMethod(v, mt=type(c.m)):
    return isinstance(v, mt)
del c


def isModule(v):
    return isinstance(v, type(sys))
...
if isPy3:
    ...
    def asUnicodeEx(v, enc='utf8'):
        return v if isinstance(
            v,
            str) else v.decode(enc) if isinstance(
            v,
            bytes) else str(v)
    ...
    def asUnicodeEx(v, enc='utf8'):
        return v if isinstance(
            v,
            unicode) else v.decode(enc) if isinstance(
            v,
            str) else unicode(v)
    ...
    def isNonPrimitiveInstance(x):
        return isinstance(
            x,
            types.InstanceType) or not isinstance(
            x,
            (float,
             int,
             long,
...
try:
    ...
    def rl_glob(
        ...
        if c or r == []:
            r += list(map(lambda x,
                          D=_archivepfx,
                          pjoin=pjoin: pjoin(_archivepfx,
                                             x),
                          list(filter(lambda x,
                                      pfn=pfn,
                                      fnmatch=fnmatch: fnmatch(x,
                                                               pfn),
...
def recursiveImport(modulename, baseDir=None, noCWD=0, debug=0):
    ...
    normalize = lambda x: os.path.normcase(
        os.path.abspath(
            os.path.normpath(x)))
    ...
    try:
        try:
            ...
            e = sys.exc_info()
            msg = "Exception raised while importing '%s': %s" % (
                modulename, e[1])
            annotateException(msg)
    finally:
        sys.path = opath
...
def open_for_read(name, mode='b', urlopen=urlopen, datareader=datareader):
    ...
    try:
        ...
        try:
            return getBytesIO(
                datareader(name) if name.startswith('data:') else urlopen(name).read())
        except:
...
def rl_isdir(
    ...
    return len(
        list(
            filter(
                lambda x,
                pn=pn: x.startswith(pn),
                list(
                    __loader__._files.keys())))) > 0
    ...
    try:
        ...
        try:
            ...
            if isCompactDistro():
                ...
                dir = (
                    dir == '.' or not dir) and _archive or os.path.join(
                    _archive,
                    dir.replace(
                        '/',
                        os.sep))
            ...
            if self._data is None:
                ...
                if sys.platform[0:4] == 'java':
                    ...
                    if mode == 'RGBA':
                        ...
                        self.mode = 'RGB'
                    self._data = (
                        im.tobytes if hasattr(
                            im,
                            'tobytes') else im.tostring)()  # make pillow and PIL both happy, for now
            return self._data
        except:
...
class DebugMemo:
    ...
    def __init__(
        ...
        for n, m in sys.modules.items():
            if n == 'reportlab' or n == 'rlextra' or n[
                    :10] == 'reportlab.' or n[
                    :8] == 'rlextra.':
                v = [
                    getattr(
        ...
        for k in K:
            ...
            try:
                ...
                d = getattr(
                    m,
                    '__version__',
                    None) == vk0 and 'SAME' or 'DIFFERENT'
            except:
                m = None
                d = '??????unknown??????'
...
def commajoin(l):
    ...
    '''
    return u','.join(
        [u' ' + asUnicode(i).replace(u',', u',,') + u' ' for i in l])
