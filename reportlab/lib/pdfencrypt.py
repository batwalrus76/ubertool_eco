...
# set a constant Doc ID to allow comparison with other software like iText
CLOBBERID = 0
...
class StandardEncryption:
    ...
    def prepare(self, document, overrideID=None):
        ...
        if DEBUG:
            print(
                'StandardEncryption.prepare(...) - revision %d' %
                self.revision)
        if self.prepared:
...
def checkU(encryptionkey, U):
    ...
    if decoded != PadString:
        ...
        raise ValueError(
            "decode of U doesn't match fixed padstring (password failed)")
...
def test():
    ...
    expectedKey = '<13DDE7585D9BE366C976DDD56AF541D1>'
...
"""
    ...
    if len(argv) > 0:
        ...
        if len(argv) < 2:
            raise ValueError(
                "Must include a filename and one or more arguments!")
        if argv[0] not in known_modes:
            ...
            raise ValueError(
                "First argument must be name of the PDF input file!")
        ...
        # argument, valid license variable, invalid license variable, text for
        # print
        ...
        if verbose > 4:
            # useful if feeling paranoid and need to double check things at
            # this point...
            print("\ninfile:", infile)
