...
# set this to 1 to disable the nonvalidating fallback parser.
RequirePyRXP = 0
...
def parsexml0(xmltext, startingat=0, toplevel=1,
    ...
    if toplevel is not None:
        ...
        raise ValueError(
            "non top level entry should be at start tag: %s" %
            repr(
                xmltext[
                    :10]))
    # special case: CDATA
        ...
        if '=' not in tagcontent:
            ...
            if '"' in tagcontent:
                ...
                while stop is None:
                    ...
                    noclose = closebracket < 0
                    tagcontent = xmltext[pastfirstbracket:closebracket]
                    # not inside double quotes! (the split should have odd
                    # length)
                    if noclose or len((tagcontent + ".").split('"')) % 2:
                        stop = 1
            if noclose:
            ...
            # put a fake att name at end of last taglist entry for consistent
            # parsing
            ...
            while taglistindex < lasttaglistindex:
                ...
                if attentry[0] != '"':
                    raise ValueError(
                        "attribute value must start with double quotes" +
                        repr(attentry))
    ...
    while docontents is not None:
        ...
        if nextopenbracket < cursor:
            ...
            if name == NONAME:
                # print "no more tags for noname",
                # repr(xmltext[cursor:cursor+10])
        ...
        # is it a close bracket?
            ...
            if nextclosebracket < nextopenbracket:
                raise ValueError(
                    ...
                    repr(
                        xmltext[
                            nextopenbracket: nextopenbracket +
                            20]))
