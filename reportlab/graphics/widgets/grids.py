...
def rotatedEnclosingRect(P, angle, rect):
    ...
    L = sorted(map(parallelAxisDist, P))
    a0, a1 = L[0], L[-1]
    L = sorted(map(orthogonalAxisDist, P))
    b0, b1 = L[0], L[-1]
...
class ShadedPolygon(Widget, LineShape):
    ...
    def draw(self):
        ...
        path = definePath([('moveTo',
                            ) + P[0]] + [('lineTo',
                                          ) + x for x in P[1:]] + ['closePath'],
                          fillColor=None,
                          strokeColor=None)
        path.isClipPath = 1
        g = Group()
