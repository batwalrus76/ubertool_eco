...
"""
    ...
    def draw(self):
        ...
        tickLine = shapes.PolyLine(points=[self.x + (s * 0.15),
                                           self.y + (s * 0.35),
                                           self.x + (s * 0.35),
                                           self.y + (s * 0.15),
                                           self.x + (s * 0.35),
...
class SmileyFace(_Symbol):
    ...
    def draw(self):
        ...
        g.add(shapes.Circle(cx=self.x + (s / 2),
                            cy=self.y + (s / 2),
                            r=s / 2,
                            fillColor=self.fillColor,
                            strokeColor=self.strokeColor,
                            strokeWidth=max(s / 38.,
                                            self.strokeWidth)))

        for i in (1, 2):
            g.add(shapes.Ellipse(self.x + (s / 3) * i,
                                 self.y + (s / 3) * 2,
                                 s / 30,
                                 s / 10,
...
class StopSign(_Symbol):
    ...
    _attrMap = AttrMap(
        BASE=_Symbol,
        stopColor=AttrMapValue(
            isColorOrNone,
            desc='color of the word stop'),
    )
    ...
    def draw(self):
        ...
        outerOctagon = shapes.Polygon(points=[self.x + athird,
                                              self.y,
                                              self.x,
                                              self.y + athird,
                                              self.x,
        ...
        innerOctagon = shapes.Polygon(points=[self.x + athird + (s / 75),
                                              self.y + (s / 75),
                                              self.x + (s / 75),
                                              self.y + athird + (s / 75),
                                              self.x + (s / 75),
        ...
        if self.stopColor:
            g.add(shapes.String(self.x + (s * 0.5),
                                self.y + (s * 0.4),
                                'STOP',
...
class NoEntry(_Symbol):
    ...
    _attrMap = AttrMap(
        BASE=_Symbol,
        innerBarColor=AttrMapValue(
            isColorOrNone,
            desc='color of the inner bar'),
    )
    ...
    def draw(self):
        ...
        if innerBarColor:
            g.add(shapes.Rect(self.x + (s * 0.1),
                              self.y + (s * 0.4),
                              width=s * 0.8,
                              height=s * 0.2,
...
"""
    ...
    def demo(self):
        ...
        D.add(yn2)
        labelFontSize = 8
        D.add(shapes.String(yn.x + (yn.size / 2),
                            (yn.y - (1.2 * labelFontSize)),
                            'testValue=0',
                            fillColor=colors.black,
                            textAnchor='middle',
                            fontSize=labelFontSize))
        D.add(shapes.String(yn2.x + (yn2.size / 2),
...
def test():
    ...
    cb.y = 530
    D.add(cb)
    D.add(shapes.String(cb.x + (cb.size / 2),
    ...
    tb.y = 530
    D.add(tb)
    D.add(shapes.String(tb.x + (tb.size / 2),
    ...
    D.add(shapes.String(yn.x + (tb.size / 2),
                        (yn.y - (1.2 * labelFontSize)),
                        tempstring,
                        fillColor=colors.black,
                        textAnchor='middle',
    ...
    ss.y = 400
    D.add(ss)
    D.add(shapes.String(ss.x + (ss.size / 2),
    ...
    ne.y = 400
    D.add(ne)
    D.add(shapes.String(ne.x + (ne.size / 2),
    ...
    sf.y = 400
    D.add(sf)
    D.add(shapes.String(sf.x + (sf.size / 2),
    ...
    ds.y = 270
    D.add(ds)
    D.add(shapes.String(ds.x + (ds.size / 2),
    ...
    na.y = 270
    D.add(na)
    D.add(shapes.String(na.x + (na.size / 2),
    ...
    ns.y = 270
    D.add(ns)
    D.add(shapes.String(ns.x + (ns.size / 2),
    ...
    a1.y = 140
    D.add(a1)
    D.add(shapes.String(a1.x + (a1.size / 2),
    ...
    a2.y = 140
    D.add(a2)
    D.add(shapes.String(a2.x + (a2.size / 2),
    ...
    fd.y = 140
    D.add(fd)
    D.add(shapes.String(fd.x + (fd.size / 2),
