...
class Flag(_Symbol):
    ...
    _attrMap = AttrMap(
        BASE=_Symbol, fillColor=AttrMapValue(
            isColor, desc='Background color'), border=AttrMapValue(
            isBoolean, 'Whether a background is drawn'), kind=AttrMapValue(
                validFlag, desc='Which flag'), )
    ...
    def availableFlagNames(self):
        ...
        return [
            x for x in self._attrMap['kind'].validate._enum if x is not None]
    ...
    def _borderDraw(self, f):
        ...
        g.insert(0, Rect(-
                         sW, -
                         sW, width=getattr(self, '_width', 2 *
                                           s) +
                         3 *
                         sW, height=getattr(self, '_height', s) +
                         2 *
                         sW, fillColor=None, strokeColor=self.strokeColor, strokeWidth=sW *
                         2))
        g.shift(x, y)
    ...
    def _Flag_USA(self):
        ...
        for starxcounter in range(6):
            for starycounter in range(5):
                ls = Star()
                ls.size = lss
                ls.x = 0 - (s / 22.0) + lss / 2.0 + \
                    s / 14.0 + starxcounter * s7
                ls.fillColor = colors.mintcream
...
# page 1
    ...
    for name in flags:
        ...
        flag.y = y
        D.add(flag)
        D.add(String(flag.x + (flag.size / 2.0),
