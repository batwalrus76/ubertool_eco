...
# history
# http://www.reportlab.co.uk/cgi-bin/viewcvs.cgi/public/reportlab/trunk/reportlab/graphics/renderbase.py
    ...
    def push(self, delta):
        ...
        for key, value in delta.items():
            if key == 'transform':  # do cumulative matrix
                ...
                # print 'statetracker ctm = (%0.2f, %0.2f, %0.2f, %0.2f, %0.2f,
                # %0.2f)' % tuple(newstate['ctm'])
...
class Renderer:
    ...
    def drawNode(self, node):
        ...
        in the tree"""
        # Undefined here, but with closer analysis probably can be handled in
        # superclass
    ...
    def drawNodeDispatcher(self, node):
        ...
        try:
            ...
            if isinstance(node, Line):
                ...
                print(
                    'DrawingError',
                    'Unexpected element %s in drawing!' %
                    str(node))
        finally:
            if not ocanvas:
                del node._canvas
    ...
    def drawGroup(self, group):
        ...
        for node in group.getContents():
            ...
            # here is where we do derived values - this seems to get
            # everything. Touch wood.
            self.fillDerivedValues(node)
            try:
                if hasattr(node, '_canvas'):
                    ocanvas = 1
