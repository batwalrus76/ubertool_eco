...
# history
# www.reportlab.co.uk/rl-cgi/viewcvs.cgi/rlextra/graphics/Csrc/renderPM/renderP.py
__version__ = ''' $Id$ '''
...
def Color2Hex(c):
    # assert isinstance(colorobj, colors.Color) #these checks don't work well
    # RGB
    if c:
        return ((0xFF & int(255 *
                            c.red)) << 16) | ((0xFF & int(255 *
                                                          c.green)) << 8) | (0xFF & int(255 *
                                                                                        c.blue))
    return c
...
def _setFont(gs, fontName, fontSize):
    try:
        ...
        try:
            ...
            raise RenderPMError(
                "Can't setFont(%s) missing the T1 files?\nOriginally %s: %s" %
                (fontName, s1, s2))
        gs.setFont(fontName, fontSize)
...
# constant for drawing circular arcs w/ Beziers
    ...
    def saveToFile(self, fn, fmt=None):
        ...
        if fmt is None:
            if not isinstance(fn, str):
                raise ValueError(
                    "Invalid value '%s' for fn when fmt is None" %
                    ascii(fn))
    ...
    def bezierArcCCW(self, cx, cy, rx, ry, theta0, theta1):
        ...
        if abs(angularExtent) <= 90.0:  # we just need one fragment
            ...
            for ii in range(Nfrag):
                ...
                # hmm.. is I wonder if this is precise enought
                b = a + fragAngle
        ...
        for (a, b) in angleList:
            if not ctrlpts:  # first time
                [(x0, y0), pts] = self._bezierArcSegmentCCW(
                    cx, cy, rx, ry, a, b)
                ctrlpts.append(pts)
            else:
                [(tmpx, tmpy), pts] = self._bezierArcSegmentCCW(
                    cx, cy, rx, ry, a, b)
                ctrlpts.append(pts)
...
def drawToPIL(
    ...
    return drawToPMCanvas(
        ...
        bg=bg,
        configPIL=configPIL,
        showBoundary=showBoundary).toPIL()
...
def test(outDir='pmout', shout=False):
    ...
    if errs:
        html[0] = html[0].replace(
            '</h1>',
            ' <a href="#errors" style="color: red">(errors)</a></h1>')
