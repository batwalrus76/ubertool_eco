...
# history
# http://www.reportlab.co.uk/cgi-bin/viewcvs.cgi/public/reportlab/trunk/reportlab/graphics/charts/utils.py
...
class DrawTimeCollector(object):
    ...
    def rectDrawTimeCallback(node, canvas, renderer, **kwds):
        ...
        D = kwds.copy()
        D['rect'] = DrawTimeCollector.transformAndFlatten(
            A, ((x1, y1), (x2, y2)))
        return D
...
def makeCircularString(
    ...
    if textAnchor != 'start':
        if textAnchor == 'middle':
            ...
            phi += sig * float(numericXShift(textAnchor,
                                             text,
                                             width,
                                             fontName,
                                             fontSize,
    ...
    for letter in text:
        ...
        h.translate(
            ...
            y +
            sin(phi) *
            radius)  # translate to radius and angle
        # rotate as needed
...
class CustomDrawChanger:
    ...
    def __call__(self, change, obj):
        if change:
            ...
            assert isinstance(
                self.store, dict), '%s.changer should return a dict of changed attributes' % self.__class__.__name__
