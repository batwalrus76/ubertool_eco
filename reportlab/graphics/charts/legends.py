...
# history
# http://www.reportlab.co.uk/cgi-bin/viewcvs.cgi/public/reportlab/trunk/reportlab/graphics/charts/legends.py
...
class SubColProperty(PropHolder):
    dividerLines = 0
    _attrMap = AttrMap(
        minWidth=AttrMapValue(
            isNumber, desc="minimum width for this subcol"), rpad=AttrMapValue(
            isNumber, desc="right padding for this subcol"), align=AttrMapValue(
                OneOf(
                    'left', 'right', 'center', 'centre', 'numeric'), desc='alignment in subCol'), fontName=AttrMapValue(
                        isString, desc="Font name of the strings"), fontSize=AttrMapValue(
                            isNumber, desc="Font size of the strings"), leading=AttrMapValue(
                                isNumber, desc="leading for the strings"), fillColor=AttrMapValue(
                                    isColorOrNone, desc="fontColor"), underlines=AttrMapValue(
                                        EitherOr(
                                            (NoneOr(
                                                isInstanceOf(Line)), SequenceOf(
                                                    isInstanceOf(Line), emptyOK=0, lo=0, hi=0x7fffffff))), desc="underline definitions"), overlines=AttrMapValue(
                                                        EitherOr(
                                                            (NoneOr(
                                                                isInstanceOf(Line)), SequenceOf(
                                                                    isInstanceOf(Line), emptyOK=0, lo=0, hi=0x7fffffff))), desc="overline definitions"), dx=AttrMapValue(
                                                                        isNumber, desc="x offset from default position"), dy=AttrMapValue(
                                                                            isNumber, desc="y offset from default position"), )
...
class Legend(Widget):
    ...
    _attrMap = AttrMap(
        ...
        colEndCallout=AttrMapValue(
            ...
            advancedUsage=1),
        subCols=AttrMapValue(
            None,
            desc="subColumn properties"),
    ...
    def _calculateMaxBoundaries(self, colorNamePairs):
        ...
        M = [
            _getWidths(
                ...
                subCols) for i,
            m in enumerate(
                self._getTexts(colorNamePairs))]
        if not M:
            return [0, 0]
        n = max([len(m) for m in M])
        if self.variColumn:
            columnMaximum = self.columnMaximum
            return [_transMax(n, M[r:r + columnMaximum])
    ...
    def draw(self):
        ...
        for i in range(n):
            ...
            for k, lines in enumerate(T):
                ...
                y += fL
                for iy, a in (
                        (y - max(fL - fA, 0), 'underlines'), (y + fA, 'overlines')):
            ...
            if dividerLines:
                ...
                if ((dividerLines & 1) and i %
                    columnMaximum) or ((dividerLines & 2) and not i %
                                       columnMaximum):
                    g.add(
                        Line(
                            thisx +
                ...
                if (dividerLines & 4) and (i %
                                           columnMaximum == lim or i == (n - 1)):
            ...
            for s in S:
                g.add(s)
            if self.colEndCallout and (
                i %
                columnMaximum == lim or i == (
                    n -
                    1)):
                if alignment == "left":
                    xt = thisx
...
class LineSwatch(Widget):
    ...
    _attrMap = AttrMap(
        x=AttrMapValue(
            isNumber,
            desc="x-coordinate for swatch line start point"),
        y=AttrMapValue(
            isNumber,
            desc="y-coordinate for swatch line start point"),
        width=AttrMapValue(
            isNumber,
            desc="length of swatch line"),
        height=AttrMapValue(
            isNumber,
            desc="used for line strokeWidth"),
        strokeColor=AttrMapValue(
            isColorOrNone,
            desc="color of swatch line"),
        strokeDashArray=AttrMapValue(
            isListOfNumbersOrNone,
            desc="dash array for swatch line"),
    )
