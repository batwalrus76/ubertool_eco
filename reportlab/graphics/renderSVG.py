...
class SVGCanvas:
    ...
    def __init__(
        ...
        svgAttrs = dict(
            ...
            # baseProfile = "full",  #disliked in V 1.0
                    ...
                    # these suggested by Tim Roberts, as updated by
                    # peter@maubp.freeserve.co.uk
        ...
        self.groupTree = transformNode(
            ...
            "g",
            id="group",
            transform="scale(1,-1) translate(0,-%d)" %
            self.height,
            **gtkw)
    ...
    def setDash(self, array=[], phase=0):
        ...
        if isinstance(array, (float, int)):
            self.style[
                'stroke-dasharray'] = ', '.join(map(str, ([array, phase])))
    ...
    def ellipse(self, x1, y1, x2, y2, link_info=None):
        ...
        ellipse = transformNode(self.doc,
                                "ellipse",
                                cx=(x1 + x2) / 2.0,
                                cy=(y1 + y2) / 2.0,
                                rx=(x2 - x1) / 2.0,
    ...
    def startGroup(self):
        ...
        currGroup, group = self.currGroup, transformNode(
            self.doc, "g", transform="")
    ...
    def curveTo(self, x1, y1, x2, y2, x3, y3):
        self.path = self.path + \
            'C %f %f %f %f %f %f ' % (x1, y1, x2, y2, x3, y3)
