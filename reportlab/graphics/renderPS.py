...
# history
# http://www.reportlab.co.uk/cgi-bin/viewcvs.cgi/public/reportlab/trunk/reportlab/graphics/renderPS.py
__version__ = ''' $Id$ '''
...
class PSCanvas:
    ...
    def drawImage(self, image, x1, y1, x2=None, y2=None):
        ...
        if self.PostScriptLevel == 1:
            ...
            raise ValueError(
                'Unsupported Postscript Level %s' %
...
''' % (self.width, self.height))
    ...
    def drawString(self, x, y, s, angle=0):
        if self._fillColor is not None:
            ...
            if not self.code[self._fontCodeLoc]:
                ...
                self.code[
                    self._fontCodeLoc] = '(%s) findfont %s scalefont setfont' % (psName,
                                                                                 fp_str(
                                                                                     self._fontSize))
    ...
    def roundRect(self, x1, y1, x2, y2, rx=8, ry=8):
        ...
        # go with newpath for precision, does this violate any assumptions in
        # code???
    ...
    def drawArc(self, x1, y1, x2, y2, startAng=0, extent=360, fromcenter=0):
        ...
        # print "x1,y1,x2,y2,startAng,extent,fromcenter",
        # x1,y1,x2,y2,startAng,extent,fromcenter
        ...
        if self._strokeColor is not None:
            ...
            (startx, starty) = (cx +
                                rx *
                                math.cos(startAngleRadians), cy +
                                ry *
                                math.sin(startAngleRadians))
    ...
    def _drawImageLevel1(self, image, x1, y1, x2=None, y2=None):
        # Postscript Level1 version available for fallback mode when Level2
        # doesn't work
        ...
        # For now let's start with 24 bit RGB images (following piddlePDF
        # again)
        component_depth = 8
        ...
        self.code.extend([
            'gsave',
            # need to start are lower left of image
            '%s %s translate' % (x1, -y1 - drawheight),
            '%s %s scale' % (drawwidth, drawheight),
            # scanline by multiples of image width
        ...
        # piddlePDF again
        rawimage = (
            myimage.tobytes if hasattr(
                myimage,
                'tobytes') else myimage.tostring)()
