...
def _hLine(canvLine, scp, ecp, y, hBlocks, FUZZ=rl_config._FUZZ):
    ...
    if not hBlocks or scp >= hBlocks[-1][1] - \
            FUZZ or ecp <= hBlocks[0][0] + FUZZ:
...
def spanFixDim(V0, V, spanCons, lim=None, FUZZ=rl_config._FUZZ):
    # assign required space to variable rows equally to existing calculated
    # values
    M = {}
    if not lim:
...
class Table(Flowable):
    ...
    def __init__(
        ...
        self._longTableOptimize = getattr(
            self,
            '_longTableOptimize',
        ...
        if not (nrows and ncols):
            if emptyTableAction == 'error':
                raise ValueError(
                    "%s must have at least a row and column" %
                    self.identity())
                ...
                raise ValueError(
                    '%s bad emptyTableAction: "%s"' %
                    (self.identity(), emptyTableAction))
            return

        # we need a cleanup pass to ensure data is strings - non-unicode and
        # non-null
        if normalizedData:
        ...
        if not _seqCW:
            ...
            if rl_config.allowShortTableRows and isinstance(colWidths, list):
                ...
                raise ValueError(
                    "%s data error - %d columns in data but %d in column widths" %
                    (self.identity(), ncols, len(colWidths)))
        if not _seqRH:
            rowHeights = nrows * [rowHeights]
        elif len(rowHeights) != nrows:
            raise ValueError(
                "%s data error - %d rows in data but %d in row heights" %
        ...
        for i, d in enumerate(data):
            ...
            if n != ncols:
                if rl_config.allowShortTableRows and isinstance(d, list):
                    ...
                    raise ValueError(
                        "%s expected %d not %d columns in row %d!" %
    ...
    def __repr__(self):
        ...
        cv = cv.replace("\n", "\n  ")
        return "%s(\n rowHeights=%s,\n colWidths=%s,\n%s\n) # end table" % (
            self.__class__.__name__, r, c, cv)
    ...
    def identity(self, maxLen=30):
        ...
        return "<%s@0x%8.8X %s rows x %s cols%s>%s" % (
            self.__class__.__name__, id(self), nr, nc, tallest, vx)
            ...
            while None in W:
                ...
                for i, Vi in enumerate(V):
                    ...
                    if ji in colSpanCells and not span:
                        ...
                        if t is None:
                            raise ValueError(
                                "Flowable %s in cell(%d,%d) can't have auto width\n%s" %
    ...
    def _elementWidth(self, v, s):
        if isinstance(v, (list, tuple)):
            ...
            if hasattr(v, 'width') and isinstance(v.width, (int, float)):
                return v.width
            if hasattr(
                    v, 'drawWidth') and isinstance(
                    v.drawWidth, (int, float)):
                return v.drawWidth
    ...
    def _calc_height(self, availHeight, availWidth, H=None, W=None):
        ...
        if None in H:
            canv = getattr(self, 'canv', None)
            saved = None
            # get a handy list of any cells which span rows. should be ignored
            # for sizing
            if self._spanCmds:
            ...
            while None in H:
                ...
                for j, (v, s, w) in enumerate(
                        list(zip(V, S, W))):  # value, style, width (lengths must match)
                    ji = j, i
                    span = spanRanges.get(ji, None)
                    if ji in rowSpanCells and not span:
                        # don't count it, it's either occluded or unreliable
                        continue
                    else:
                        if isinstance(v, (tuple, list, Flowable)):
                            ...
                            if w is None and not self._canGetWidth(v):
                                raise ValueError(
                                    "Flowable %s in cell(%d,%d) can't have auto width in\n%s" %
                                    (v[0].identity(30), i, j, self.identity(30)))
                            if canv:
                                canv._fontname, canv._fontsize, canv._leading = s.fontname, s.fontsize, s.leading or 1.2 * \
                                    s.fontsize
        ...
        assert abs(height) < 1e-8, '!!!!!%s\ninternal height error height=%r hmax=%d Sum(H[:%d])=%r\nH=%r\nrowPositions=%r' % (
            self.identity(), height, hmax, hmax, self._height, H[:hmax], self._rowpositions)
        self._hmax = hmax
    ...
    def _calc(self, availWidth, availHeight):
        ...
        if self._spanCmds:
            # now work out the actual rect for each spanned cell from the
            # underlying grid
            self._calcSpanRects()
    ...
    def _calcSpanRects(self):
        ...
        longTable = self._longTableOptimize
        if spanRects and (
                longTable and hmax == self._hmax_spanRects or not longTable):
            return
        colpositions = self._colpositions
    ...
    def _addCommand(self, cmd):
        if cmd[0] in ('BACKGROUND', 'ROWBACKGROUNDS', 'COLBACKGROUNDS'):
            ...
            cmd = list(cmd)
            if len(cmd) < 5:
                raise ValueError('bad line command ' + ascii(cmd))

            # determine line cap value at position 5. This can be str or
            # numeric.
            if len(cmd) < 6:
                cmd.append(1)
            else:
    ...
    def _drawLines(self):
        ...
        self.canv.saveState()
        for op, (sc, sr), (ec,
                           er), weight, color, cap, dash, join, count, space in self._linecmds:
            ...
            if er < 0:
                er = er + self._nrows
            if cap is not None and ccap != cap:
    ...
    def _drawBox(self, start, end, weight, color, count, space):
        ...
        self._drawHLines(
            (sc, er + 1), (ec, er + 1), weight, color, count, space)
        self._drawVLines((sc, sr), (sc, er), weight, color, count, space)
        self._drawVLines(
            (ec + 1, sr), (ec + 1, er), weight, color, count, space)
    ...
    def _drawHLines(self, start, end, weight, color, count, space):
        ...
        if count == 1:
            ...
            lf = lambda x0, y0, x1, y1, canvLine=canvLine, ws=weight + \
                space, count=count: _multiLine(x0, x1, y0, canvLine, ws, count)
            for y in rp:
    ...
    def _drawHLinesB(self, start, end, weight, color, count, space):
        ...
        ec, er = end
        self._drawHLines(
            (sc, sr + 1), (ec, er + 1), weight, color, count, space)
    ...
    def _drawVLines(self, start, end, weight, color, count, space):
        ...
        canvLine = lambda y0, x0, y1, x1, _line=self.canv.line: _line(
            x0, y0, x1, y1)
        if count == 1:
            for x in cp:
    ...
    def _drawVLinesA(self, start, end, weight, color, count, space):
        ...
        ec, er = end
        self._drawVLines(
            (sc + 1, sr), (ec + 1, er), weight, color, count, space)
    ...
    def _cr_0(self, n, cmds):
        for c in cmds:
            c = tuple(c)
            (sc, sr), (ec, er) = c[1:3]
            if isinstance(sr, strTypes) or sr >= n:
                continue
            if er >= n:
                er = n - 1
    ...
    def _splitRows(self, availHeight):
        ...
        R0 = self.__class__(
            ...
            rowHeights=self._argH[
                :n],
            repeatRows=repeatRows,
            repeatCols=repeatCols,
            splitByRow=splitByRow,
            normalizedData=1,
            cellStyles=self._cellStyles[
                :n],
            ident=ident)
        ...
        for op, (sc, sr), (ec,
            ...
            if isinstance(sr, strTypes) and sr.startswith('split'):
                A.append(
                    (op, (sc, sr), (ec, sr), weight, color, cap, dash, join, count, space))
                if sr == 'splitlast':
                    sr = er = n - 1
                elif sr == 'splitfirst':
                    sr = n
            ...
            if op in ('BOX', 'OUTLINE', 'GRID'):
                if sr < n and er >= n:
                    ...
                    A.append(
                        ('LINEABOVE', (sc, sr), (ec, sr), weight, color, cap, dash, join, count, space))
                    A.append(
                        ('LINEBEFORE', (sc, sr), (sc, er), weight, color, cap, dash, join, count, space))
                    A.append(
                        ('LINEAFTER', (ec, sr), (ec, er), weight, color, cap, dash, join, count, space))
                    A.append(
                        ('LINEBELOW', (sc, er), (ec, er), weight, color, cap, dash, join, count, space))
                    if op == 'GRID':
                        A.append(
                            ('LINEBELOW',
                             (sc,
                              n - 1),
                                ...
                                space))
                        A.append(
                            ('LINEABOVE', (sc, n), (ec, n), weight, color, cap, dash, join, count, space))
                        A.append(
                            ('INNERGRID', (sc, sr), (ec, er), weight, color, cap, dash, join, count, space))
                else:
                    A.append(
            ...
            if r is not None:
                ...
                if y1 != y2:
                    ...
                    while True:
                        if y > ymax:
                            break
                        # split at position y is impossible because of
                        # overlapping rowspan
                        impossible[y] = None
                        y += 1
    ...
    def _getFirstPossibleSplitRowPosition(self, availHeight):
        ...
        h = 0
        n = 1
        # from this point of view 0 is the first position where the table may
        # *always* be splitted
        split_at = 0
    ...
    def draw(self):
        ...
        if not self._spanCmds:
            ...
            for row, rowstyle, rowpos, rowheight in zip(
                self._cellvalues, self._cellStyles, self._rowpositions[
                    1:], self._rowHeights):
                for cellval, cellstyle, colpos, colwidth in zip(
                    row, rowstyle, self._colpositions[
                        :-1], self._colWidths):
                    self._drawCell(
            ...
            for rowNo in range(self._nrows):
                for colNo in range(self._ncols):
                    ...
                    if cellRect is not None:
                        ...
                        self._drawCell(
                            cellval, cellstyle, (x, y), (width, height))
        self._drawLines()
    ...
    def _drawCell(self, cellval, cellstyle, pos, size):
        ...
        if isinstance(cellval, (tuple, list, Flowable)):
            ...
            if valign == 'TOP':
                ...
                y = rowpos + \
                    (rowheight + cellstyle.bottomPadding - cellstyle.topPadding + h) / 2.0
            if cellval:
            ...
            if valign == 'BOTTOM':
                ...
                # tim roberts pointed out missing fontsize correction
                # 2004-10-04
