...
# history
# http://www.reportlab.co.uk/cgi-bin/viewcvs.cgi/public/reportlab/trunk/reportlab/platypus/flowables.py
__version__ = ''' $Id$ '''
__doc__ = """
...
class Flowable:
    ...
    def __init__(self):
        ...
        # these are hints to packers/frames as to how the floable should be
        # positioned
    ...
    def identity(self, maxLen=None):
        ...
        if r and maxLen:
            r = r[:maxLen]
        return "<%s at %s%s>%s" % (
            self.__class__.__name__, hex(id(self)), self._frameName(), r)
...
class Preformatted(Flowable):
    ...
    def minWidth(self):
        ...
        return max([stringWidth(line, fontName, fontSize)
...
def _flowableSublist(V):
    ...
    assert not [
        x for x in V if isinstance(
            x, LCActionFlowable)], 'LCActionFlowables not allowed in sublists'
    return V
...
class KeepTogether(_ContainerSpace, Flowable):
    ...
    def __repr__(self):
        ...
        L = "\n" + "\n".join(L)
        L = L.replace("\n", "\n  ")
        return "%s(%s,maxHeight=%s)" % (
            self.__class__.__name__, L, self._maxHeight)
    ...
    def identity(self, maxLen=None):
        msg = "<%s at %s%s> containing :%s" % (self.__class__.__name__, hex(
            id(self)), self._frameName(), "\n".join([f.identity() for f in self._content]))
        if maxLen:
            return msg[0:maxLen]
        else:
            return msg
...
class HRFlowable(Flowable):
    ...
    def draw(self):
        ...
        canv.setLineCap(
            {'butt': 0, 'round': 1, 'square': 2}[self.lineCap.lower()])
...
# Abstract some common container like behaviour
...
class PTOContainer(_Container, Flowable):
    ...
    def split(self, availWidth, availHeight):
        ...
        if len(SS) > 1:
            ...
            R1 = C[:i] + R1I + T + F
            R2 = Hdr + R2I + C[i:]
        T = R1 + [PTOContainer(R2,
                               [copy(x) for x in I.trailer],
                               [copy(x) for x in I.header])]
        return T
...
class KeepInFrame(_Container, Flowable):
    ...
    def wrap(self, availWidth, availHeight):
        ...
        W, H = _listWrapOn(
            self._content, maxWidth, self.canv, fakeWidth=fakeWidth)
        if (mode == 'error' and (
                W > maxWidth + _FUZZ or H > maxHeight + _FUZZ)):
            ident = 'content %sx%s too large for %s' % (
                W, H, self.identity(30))
            ...
            if H < min(
                    0.95 * maxHeight,
                    maxHeight - 10) or H >= maxHeight + _FUZZ:
...
class FrameSplitter(NullDraw):
    ...
    def wrap(self, aW, aH):
        ...
        if aH < self.gap + self.required - _FUZZ:
            # we are going straight to the nextTemplate with no attempt to
            # modify the frames
            G.append(PageBreak())
        else:
            ...
            if templates is None:
                raise LayoutError(
                    '%s called in non-doctemplate environment' %
                    self.identity())
            ...
            if N:
                raise LayoutError(
                    ...
                    (self.identity(), N, T.id, T, [
                        f.id for f in T.frames]))
...
class DDIndenter(Flowable):
    ...
    def drawOn(self, canv, x, y, _sW=0):
        self._flowable.drawOn(canv, x +
                              self._leftIndent, y, max(0, _sW -
                                                       self._leftIndent -
                                                       self._rightIndent))

    def wrap(self, aW, aH):
        w, h = self._flowable.wrap(
            aW - self._leftIndent - self._rightIndent, aH)
    ...
    def identity(self, maxLen=None):
        return '%s containing %s' % (
            self.__class__.__name__, self._flowable.identity(maxLen))
...
class ListFlowable(_Container, Flowable):
    ...
    def __init__(self,
        ...
        if style:
            if not isinstance(style, ListStyle):
                raise ValueError(
                    '%s style argument not a ListStyle' %
                ...
                if not params:
                    ...
                    for i, z in enumerate(f._flowables):
                        if isinstance(z, LIIndenter):
                            raise ValueError(
                                'LIIndenter not allowed in ListItem')
                        yield i == 0, z
                else:
                    params = params.copy()
    ...
    def _getContent(self):
        ...
        for d, f in self._flowablesIter():
            ...
            if d:
                aS(self._makeLIIndenter(f,
                                        bullet=self._makeBullet(value),
                                        params=fparams))
                if inc:
                    value += inc
            elif isinstance(f, LIIndenter):
