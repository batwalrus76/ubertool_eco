...
class paragraphEngine:
    ...
    def __init__(self, program=None):
        ...
        if program is None:
            program = []
        # for handling underlining and hyperlinking, etc
        self.lineOpHandlers = []
        self.program = program
        self.indent = self.rightIndent = 0.0
        # adjust this to add more indentation for bullets, eg
        self.baseindent = 0.0
    ...
    def format(self, maxwidth, maxheight, program, leading=0):
        ...
        room = 1
        cursorcount = 0  # debug
        # heightremaining>=self.leading and remainder:
        while remainder and room:
            ...
            try:
                (lineIsFull,
                 ...
                 usedIndent,
                 maxLength,
                 justStrings) = self.fitLine(remainder,
                                             maxwidth)
            except:
...
# print "covers", currentLength, "of", maxwidth
            # last declared alignment for this line used
            ...
            if alignment == TA_LEFT:
                ...
                if remainder and lineIsFull:
                    if justStrings:
                        line = simpleJustifyAlign(
                            line,
                            currentLength,
                            maxLength)
                    else:
                        line = self.justifyAlign(
                            line,
                            currentLength,
                            maxLength)
                else:
                    ...
                    if debug:
                        print(
                            "no justify because line is not full or end of para")
            else:
...
# return float(stringWidth(word, self.fontName, self.fontSize))
    ...
    def fitLine(self, program, totalLength):
        ...
        # fit words onto a line up to maxlength, adding spaces and respecting
        # extra space
        from reportlab.pdfbase.pdfmetrics import stringWidth
        ...
        while not done and cursor < maxcursor:
            ...
            if isinstance(opcode, str) or hasattr(opcode, 'width'):
                ...
                if newlength > maxLength and not first:
                    ...
                    # print "   currentLength", currentLength, "newlength",
                    # newlength, "maxLength", maxLength
                    done = 1
                    lineIsFull = 1
                else:
                ...
                if indicator == "nextLine":
                    ...
                    # adjust for base indent (only at format time -- only
                    # execute once)
                    ...
                    (i, handler) = opcode
                    line.append(opcode)
                    self.lineOpHandlers = self.lineOpHandlers + \
                        [handler]  # fresh copy
                    ...
                    raise ValueError(
                        "at format time don't understand indicator " +
                        repr(indicator))
            else:
                raise ValueError(
                    "op must be string, float, instance, or tuple " +
                    repr(opcode))
            if not done:
                cursor = cursor + 1
                #first = 0
...
# return result
    ...
    def shrinkWrap(self, line):
        # for non justified text, collapse adjacent text/shift's into single
        # operations
        result = []
        index = 0
        maxindex = len(line)
            ...
            for index in rline:
                ...
                if isinstance(this, str) or \
                   ...
                   hasattr(this, 'width') or hasattr(next, 'width'):
                    doswap = 0
                # only swap two tuples if the second one is an end operation
                # and the first is something else
                elif isinstance(this, tuple):
                    thisindicator = this[0]
                    if isinstance(next, tuple):
                        nextindicator = next[0]
                        doswap = 0
                        if (nextindicator == "endLineOperation" and thisindicator !=
                                "endLineOperation" and thisindicator != "lineOperation"):
                ...
                if not indented:
                    ...
                    for handler in self.lineOpHandlers:
                        ...
                        handler.start_at(
                            ...
                            self,
                            canvas,
                            textobject)  # start underlining the next
                indented = 1
                ...
                if indicator == "nextLine":
                    ...
                    for handler in self.lineOpHandlers:
                        handler.end_at(
                            ...
                            self,
                            canvas,
                            textobject)  # finish, eg, underlining this line
                    ...
                    if bulletmin > thislineindent:
                        # if debug: print "BULLET IS BIG", bullet, bulletmin,
                        # thislineindent
                    ...
                    raise ValueError(
                        "don't understand indicator " +
                        repr(indicator))
            else:
                raise ValueError(
                    "op must be string float or tuple " +
                    repr(opcode))
        laststate = self.__dict__.copy()
        # self.resetState(startstate)
        self.__dict__.update(startstate)
    ...
    def wrap(self, availableWidth, availableHeight):
        ...
        if maxlength < spacewidth:
            return (
                ...
                rightIndent +
                firstindent,
                availableHeight)  # need something wider than this!
        ...
        if remainder:
            result = (
                availableWidth,
                availableHeight +
                leading)  # need to split
        else:
            result = (availableWidth, heightused)
        # if debug: print "wrap is", (availableWidth, availableHeight), result,
        # len(lines)
        return result
        ...
        while count < nlines:
            ...
            if alignment == TA_LEFT:
                ...
                if count < nlines and nwords > 1:
                    # patch from doug@pennatus.com, 9 Nov 2002, no extraspace
                    # on last line
                    textobject.setWordSpace(
                        (basicWidth - length) / (nwords - 1.0))
                else:
...
class Para(Flowable):
    ...
    def __init__(
        ...
        # make sure Flowable doesn't use this unless wanted! call it style1 NOT
        # style
        self.style1 = style
    ...
    def wrap(self, availableWidth, availableHeight):
        ...
        if availableHeight <= needatleast:
            self.cansplit = 0
            # if debug:
            # print "CANNOT COMPILE, NEED AT LEAST", needatleast, 'AVAILABLE',
            # availableHeight
        ...
        if not self.formattedProgram:
            (formattedProgram,
             remainder,
             laststate,
             heightused) = p.format(availableWidth,
                                    availableHeight,
                                    program,
                                    leading)
        ...
        if remainder:
            ...
            # print "heightused", heightused, "available", availableHeight,
    ...
    def compileProgram(self, parsedText, program=None):
        ...
        # now look for a place where to insert the unindent after the first
        # line
        if style.firstLineIndent:
            count = 0
            for x in program:
...
##        if dump: stop
    ...
    def compile_ul(self, attdict, content, extra, program, tagname="ul"):
        ...
        for e in content:
            if isinstance(e, str):
                if e.strip():
                    raise ValueError(
                        "don't expect CDATA between list elements")
            elif isinstance(e, tuple):
                (tagname, attdict1, content1, extra) = e
                if tagname != "li":
                    raise ValueError(
                        "don't expect %s inside list" %
                        repr(tagname))
                newatts = atts.copy()
                if attdict1:
    ...
    def compile_font(self, attdict, content, extra, program):
        ...
        if "size" in attdict:
            # size = float(attdict["size"]) # really should convert int, cm etc
            # here!
            size = attdict["size"]
            program.append(("size", size))
        for e in content:
    ...
    def compile_bullet(self, attdict, content, extra, program):
        # eventually should allow things like images and graphics in bullets
        # too XXXX
    ...
    def compile_para(
        ...
        newstyle = SimpleStyle(
            name="rml2pdf internal embedded style",
            parent=style)
        ...
        self.baseindent = thepara.baseindent = mystyle.leftIndent + \
            self.baseindent
        ...
        if bl is None:
            if tagname == "ul":
                if typ == "disc":
                    ...
                    raise ValueError(
                        "unordered list type %s not implemented" %
                        repr(typ))
                ...
                if typ == "1":
                    ...
                    raise ValueError(
                        "ordered bullet type %s not implemented" %
                        repr(typ))
            else:
...
class PageNumberObject:
    ...
    def __init__(self, example="XXX"):
        # XXX SHOULD ADD THE ABILITY TO PASS IN EXAMPLES
...
def EmbedInRml2pdf():
        ...
        def translate(self, nodetuple, controller, context, overrides):
            ...
            if result is None:
                result = Para(
                    ...
                    content,
                    context=context,
                    bulletText=bulletText)  # possible ref loop on context, break later
            return result
    ...
    class ulMapper(MapNode):
        ...
        def translate(self, nodetuple, controller, context, overrides):
            ...
            return theParaMapper.translate(
                thepara,
                controller,
                context,
                overrides)
...
def handleSpecialCharacters(engine, text, program=None):
    ...
    for fragment in amptext:
        ...
        if sfragment and fragment:
            # reader 3 used to go nuts if you don't special case the last frag,
            # but it's fixed?
