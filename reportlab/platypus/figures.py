...
# history
# http://www.reportlab.co.uk/cgi-bin/viewcvs.cgi/public/reportlab/trunk/reportlab/platypus/figures.py
...
class Figure(Flowable):
    ...
    def __init__(self, width, height, caption="",
        ...
        # Larry Meyn's fix - otherwise they all get the number of the last
        # chapter.
    ...
    def _getCaptionPara(self):
        ...
        if self._captionData != (
            ...
            self.captionStyle = ParagraphStyle(
                ...
                alignment=TA_CENTER)
            # must build paragraph now to get sequencing in synch with rest of
            # story
    ...
    def wrap(self, availWidth, availHeight):
        ...
        if self.caption:
            self._getCaptionPara()
            w, h = self.captionPara.wrap(
                self.width, availHeight - self.figureHeight)
            self.captionHeight = h + \
                (self.captionGap or 0.5 * self.captionSize)
...
if _hasPageCatcher:
    ...
    class cachePageCatcherFigureNonA4(FlexFigure, PageCatcherCachingMixIn):
        ...
        def __init__(
            ...
            self.pageNo = pageNo
            self.formName = self.getFormName(
                self.filename,
                self.pageNo) + '_' + str(pageNo)
        ...
        def drawFigure(self):
            ...
            if not self.canv.hasForm(self.formName):
                ...
                formFileName = self.getFormName(
                    restorePath,
                    self.pageNo) + '.frm'
                if self.needsProcessing(restorePath, self.pageNo):
                    # print 'preprocessing PDF %s page %s' % (restorePath,
                    # self.pageNo)
            ...
            if not self.canv.hasForm(self.formName):
                if self.filename in self._cache:
                    ...
                    f, data = storeFormsInMemory(
                        pdf, pagenumbers=[
                            self.pageNo], prefix=self.prefix)
