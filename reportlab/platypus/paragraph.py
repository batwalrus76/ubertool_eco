...
# history
# http://www.reportlab.co.uk/cgi-bin/viewcvs.cgi/public/reportlab/trunk/reportlab/platypus/paragraph.py
__version__ = ''' $Id$ '''
...
def split(text, delim=None):
    ...
    return [
        uword for uword in (
            _wsc_re_split(text) if delim is None and u'\xa0' in text else text.split(delim))]
    ...
    for f in words:
        if hasattr(f, 'cbDefn'):
                ...
                if kind == 'anchor':
                    ...
                    if not func:
                        raise AttributeError(
                            "Missing %s callback attribute '%s'" %
                            (kind, name))
            ...
            if not xs.underline and f.underline:
                ...
                if not f.underline:
                    xs.underline = 0
                    xs.underlines.append(
                        (xs.underline_x, cur_x_s, xs.underlineColor))
            ...
            if f.link and not xs.link:
                ...
                if not f.link:
                    xs.links.append(
                        (xs.link_x, cur_x_s, xs.link, xs.linkColor))
                    xs.link = None
                    xs.linkColor = None
            ...
            if bg and not xs.backColor:
                ...
                if not bg:
                    xs.backColors.append(
                        (xs.backColor_x, cur_x_s, xs.backColor))
                    xs.backColor = None
...
def _justifyDrawParaLineX(tx, offset, line, last=0):
    ...
    if not simple:
        nSpaces = line.wordCount + \
            sum([_nbspCount(w.text) for w in line.words if not hasattr(w, 'cbDefn')]) - 1
        simple = not nSpaces
    if not simple:
        tx.setWordSpace(extraSpace / float(nSpaces))
...
def _split_blParaHard(blPara, start, stop):
    ...
    for l in lines:
        ...
        if l is not lines[-1]:
            i = len(f) - 1
            while i >= 0 and hasattr(
                    f[i],
                    'cbDefn') and not getattr(
                    f[i].cbDefn,
                    'width',
                    0):
                i -= 1
...
def _drawBullet(canvas, offset, cur_y, bulletText, style, rtl):
    ...
    if not rtl:
        ...
        bulletWidth = stringWidth(
            bt,
            style.bulletFontName,
            style.bulletFontSize)
        width = rtl[0]
        bulletStart = width + style.rightIndent - \
            (style.bulletIndent + bulletWidth)
        tx2 = canvas.beginText(
            bulletStart,
            cur_y +
            getattr(
        ...
        if isinstance(bulletText, strTypes):
            ...
            for f in bulletText:
                bulletWidth = bulletWidth + \
                    stringWidth(f.text, f.fontName, f.fontSize)
        bulletLen = style.bulletIndent + \
            bulletWidth + 0.6 * style.bulletFontSize
...
def _do_under_line(i, t_off, ws, tx, lm=-0.125):
    y = tx.XtraState.cur_y - i * tx.XtraState.style.leading + \
        lm * tx.XtraState.f.fontSize
    textlen = tx._canvas.stringWidth(
        ' '.join(
            tx.XtraState.lines[i][1]),
        tx._fontname,
        tx._fontsize)
...
def _do_link_line(i, t_off, ws, tx):
    ...
    # 8.0 factor copied from para.py
...
def textTransformFrags(frags, style):
    ...
    if tt:
        ...
        if tt == 'lowercase':
            ...
            raise ValueError(
                'ParaStyle.textTransform value %r is invalid' %
                style.textTransform)
        n = len(frags)
        if n == 1:
        ...
        if calcBounds:
            ...
            if getattr(cbDefn, 'width', 0):
                descent, ascent = imgVRange(
                    imgNormV(
                        cbDefn.height, fontSize), cbDefn.valign, fontSize)
            else:
...
class Paragraph(Flowable):
    ...
    def _setup(self, text, style, bulletText, frags, cleaner):
        ...
        # turn this on to see a pretty one with all the margins etc.
        self.debug = 0
    ...
    def split(self, availWidth, availHeight):
        ...
        P2 = self.__class__(
            ...
            frags=func(
                ...
                s,
                n))
        # propagate attributes that might be on self; suggestion from Dirk
        # Holtwick
    ...
    def breakLines(self, width):
        ...
        # for bullets, work out width and ensure we wrap the right amount onto
        # line one
        ...
        frags = self.frags
        nFrags = len(frags)
        if nFrags == 1 and not hasattr(
                frags[0],
                'cbDefn') and not style.endDots:
            f = frags[0]
            fontSize = f.fontSize
            fontName = f.fontName
            ...
            while words:
                word = words.pop(0)
                # this underscores my feeling that Unicode throughout would be
                # easier!
                wordWidth = stringWidth(
                    word,
                    fontName,
                    fontSize,
                    self.encoding)
            ...
            while _words:
                ...
                # if the current width is non-negative or the previous thing
                # was a deliberate lineBreak
                lineBreak = hasattr(f, 'lineBreak')
                if not lineBreak and newWidth > maxWidth and not isinstance(
                        w,
                        _SplitList) and splitLongWords:
                    nmw = min(lineno, maxlineno)
                    ...
                    if calcBounds:
                        ...
                        if getattr(cbDefn, 'width', 0):
                            minDescent, maxAscent = imgVRange(
                                imgNormV(
                                    cbDefn.height, fontSize), cbDefn.valign, maxSize)
                        else:
                            maxAscent, minDescent = getAscentDescent(
                                g.fontName, maxSize)
                    else:
    ...
    def drawPara(self, debug=0):
        ...
        if nLines > 0:
            ...
            noJustifyLast = not (
                hasattr(
                    self,
                    '_JustifyLast') and self._JustifyLast)
            ...
            if blPara.kind == 0:
                ...
                if bulletText:
                    offset = _drawBullet(
                        ...
                        bulletText,
                        style,
                        rtl=style.wordWrap == 'RTL' and self._wrapWidths or False)
                ...
                if f.underline or f.link or f.strike or style.endDots:
                    ...
                    if dpl != _justifyDrawParaLine:
                        ws = 0
                    underline = f.underline or (
                        f.link and platypus_link_underline)
                    strike = f.strike
                    link = f.link
                    if underline:
                    ...
                    # now the middle of the paragraph, aligned with the left
                    # margin which is our origin.
                ...
                # now the middle of the paragraph, aligned with the left margin
                # which is our origin.
...
if __name__ == '__main__':  # NORUNTESTS
    ...
    def dumpParagraphFrags(P):
        ...
        n = len(frags)
        for l in range(n):
            print("frag%d: '%s' %s" % (l, frags[l].text, ' '.join(
                ['%s=%s' % (k, getattr(frags[l], k)) for k in frags[l].__dict__ if k != text])))
...
'''
    ...
    if flagged(2):
        P = Paragraph(
            """Price<super><font color="red">*</font></super>""",
            styleSheet['Normal'])
        dumpParagraphFrags(P)
        w, h = P.wrap(24, 200)
        dumpParagraphLines(P)
...
umfassend zu sein."""
    ...
    if flagged(5):
        text = '<para> %s <![CDATA[</font></b>& %s < >]]></para>' % (
            chr(163), chr(163))
        P = Paragraph(text, styleSheet['Code'])
        dumpParagraphFrags(P)
        w, h = P.wrap(6 * 72, 9.7 * 72)
