...
# history
# http://www.reportlab.co.uk/cgi-bin/viewcvs.cgi/public/reportlab/trunk/reportlab/pdfbase/pdfutils.py
__version__ = ''' $Id$ '''
__doc__ = ''
...
def makeA85Image(filename, IMG=None):
    ...
    assert len(raw) == imgwidth * imgheight * \
        _mode2bpp[img.mode], "Wrong amount of data for image"
...
def cacheImageFile(filename, returnInMemory=0, IMG=None):
    ...
    cachedname = os.path.splitext(
        filename)[0] + (rl_config.useA85 and '.a85' or '.bin')
...
def _AsciiHexDecode(input):
    ...
    return ''.join([chr(int(stripped[i:i + 2], 16))
...
class _fusc:
    ...
    def decrypt(self, s):
        return ''.join(map(chr, self.__fusc(
    ...
    def __fusc(self, s):
        slen = len(s)
        return list(map(lambda x, y: x ^ y, s, list(
            map(ord, ((int(slen / self._klen) + 1) * self._k)[:slen]))))
