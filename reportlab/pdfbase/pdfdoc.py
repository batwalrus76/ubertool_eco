...
# history
# http://www.reportlab.co.uk/cgi-bin/viewcvs.cgi/public/reportlab/trunk/reportlab/pdfbase/pdfdoc.py
__version__ = ''' $Id$ '''
__doc__ = """
...
# __InternalName__ is a special attribute that can only be set by the
# Document arbitrator
...
def format(element, document, toplevel=0):
    ...
    if isinstance(element, PDFObject):
        if not toplevel and hasattr(element, __RefOnly__):
            ...
            if not rl_config.invariant and rl_config.pdfComments and hasattr(
                    element,
                    __Comment__):
...
class PDFDocument(PDFObject):
    ...
    def __init__(self,
        ...
        # mapping of internal identifier ("Page001") to PDF objectnumber and
        # generation number (34, 0)
        self.idToObjectNumberAndVersion = {}
        # mapping of internal identifier ("Page001") to PDF object (PDFPage
        # instance)
        self.idToObject = {}
    ...
    def inPage(self):
        ...
        if self.inObject is not None:
            if self.inObject == "page":
                return
            raise ValueError(
                "can't go in page already in object %s" %
                self.inObject)
        self.inObject = "page"
    ...
    def addColor(self, cmyk):
        ...
        if not sname:
            ...
            if not sname:
                raise ValueError(
                    "CMYK colour %r used without a spotName" %
                    cmyk)
            else:
                cmyk = cmyk.clone(spotName=sname)
        name = PDFName(sname)[1:]
        if name not in self.idToObject:
            # PDFArray([/Separation /name /DeviceCMYK tint_tf])
    ...
    def getAvailableFonts(self):
        ...
        # the standard 14 are also always available! (even if not initialized
        # yet)
    ...
    def format(self):
        ...
        if counter - 1 != lno:
            raise ValueError(
                "counter %s doesn't match number to id dictionary %s" %
                (counter, lno))
        # now add the xref
    ...
    def getFormBBox(self, name, boxType="MediaBox"):
        ...
        if internalname in self.idToObject:
            ...
            if isinstance(theform, PDFFormXObject):
                ...
                raise ValueError(
                    "I don't understand the form instance %s" %
                    repr(name))
    ...
    def Reference(self, object, name=None):
        ...
        if name is None and (
                not iob or object.__class__ is PDFObjectReference):
            return object
        ...
        if (isBytes(s)):
            if enc is 'auto':
                try:
                    u = s.decode(
                        s.startswith(
                            codecs.BOM_UTF16_BE) and 'utf16' or 'utf8')
                    if _checkPdfdoc(u):
                        s = u.encode('pdfdoc')
                    else:
            ...
            raise ValueError(
                'PDFString argument must be str/unicode not %s' %
                type(s))
...
class ViewerPreferencesPDFDictionary(CheckedPDFDictionary):
    validate = dict(
        ...
        ViewArea=checkPDFNames(
            *
            'MediaBox CropBox BleedBox TrimBox ArtBox'.split()),
        ViewClip=checkPDFNames(
            *
            'MediaBox CropBox BleedBox TrimBox ArtBox'.split()),
        PrintArea=checkPDFNames(
            *
            'MediaBox CropBox BleedBox TrimBox ArtBox'.split()),
        PrintClip=checkPDFNames(
            *
            'MediaBox CropBox BleedBox TrimBox ArtBox'.split()),
...
def teststream(content=None):
    ...
    S = PDFStream(
        content=content,
        filters=rl_config.useA85 and [
            PDFBase85Encode,
            PDFZCompress] or [PDFZCompress])
    # nothing else needed...
...
class PDFIndirectObject(PDFObject):
    ...
    def format(self, document):
        ...
        fcontent = format(
            self.content,
            document,
            toplevel=1)   # yes this is at top level
...
class PDFObjectReference(PDFObject):
    ...
    def format(self, document):
        try:
            return pdfdocEnc(
                "%s %s R" %
                document.idToObjectNumberAndVersion[
                    self.name])
        except:
            raise KeyError(
    ...
    def check_format(self, document):
        ...
        if not self.Contents:
            ...
            if not stream:
                ...
                S = PDFStream()
                if self.compression:
                    S.filters = rl_config.useA85 and [
                        PDFBase85Encode,
                        PDFZCompress] or [PDFZCompress]
                S.content = stream
...
class PDFOutlines(PDFObject):
    ...
    def __init__(self):
        ...
        self.levelstack = []
        self.buildtree = []
        # dictionary of "closed" destinations in the outline
        self.closedict = {}
    ...
    def addOutlineEntry(
        ...
        while level < currentlevel:
            ...
            if isinstance(lastinprevious, tuple):
                ...
                raise ValueError(
                    "cannot reset existing sections: " +
                    repr(lastinprevious))
            else:
                name = lastinprevious
                sectionlist = current
    ...
    def translateNames(self, canvas, object):
        ...
        raise TypeError(
            "in outline, destination name must be string: got a %s" %
            type(object))
    ...
    def prepare(self, document, canvas):
        ...
        (self.first,
         self.last) = self.maketree(document,
                                    self.mydestinations,
                                    toplevel=1)
        self.ready = 1
    ...
    def maketree(self, document, destinationtree, Parent=None, toplevel=0):
        if toplevel:
            ...
            if Parent is None:
                raise ValueError(
                    "non-top level outline elt parent must be specified")
        ...
        for index in range(nelts):
            ...
            if isinstance(elt, dict):
                ...
                # leaf with subsections: ({name: ref}, subsections) XXXX should
                # clean up (see count(...))
                try:
                ...
                (eltobj.First,
                 eltobj.Last) = self.maketree(document,
                                              subsections,
                                              eltref)
            else:
                raise ValueError(
                    "destination tree elt should be dict or tuple, got %s" %
                    type(elt))
            try:
                [(Title, Dest)] = list(leafdict.items())
            except:
                raise ValueError(
                    "bad outline leaf dictionary, should have one entry " +
                    bytestr(elt))
...
class PDFTrueTypeFont(PDFType1Font):
    Subtype = "TrueType"
    # local_attributes = "FirstChar LastChar Widths Encoding ToUnicode
    # FontDescriptor".split() #same
...
class PDFFormXObject(PDFObject):
    ...
    def format(self, document):
        ...
        if not self.Resources:
            ...
            self.Resources = resources
        if self.compression:
            self.Contents.filters = rl_config.useA85 and [
                PDFBase85Encode,
                PDFZCompress] or [PDFZCompress]
...
class PDFImageXObject(PDFObject):
    ...
    def loadImageFromA85(self, source):
        ...
        self.colorSpace = {'/RGB': 'DeviceRGB',
                           '/G': 'DeviceGray',
                           '/CMYK': 'DeviceCMYK'}[words[7]]
...
class PDFStitchingFunction(PDFFunction):
    ...
    def Dict(self, document):
        ...
        d.update(self.otherkw)
        d["FunctionType"] = 3
        d["Functions"] = PDFArray(
            [document.Reference(x) for x in self.Functions])
