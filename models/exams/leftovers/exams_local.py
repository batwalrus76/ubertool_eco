...
exams_obj = exams_model.exams('chem_name_1',
                              'CA Almonds MLRA-17',
                              'Yes',
                              70,
                              71,
...
def exams_pi(exams_obj):
    ...
    ##########################################################################
    # Create a folder if it does not existed, where holds calculations' ou
    ##########################################################################
    ...
    def update(exams_obj):
        ...
        def residuals(k_abn, ph_out, tmper, hl_out):
            ...
            ph_adj = 6013.79 / (tmper + 273.15) + \
                23.6521 * np.log10(tmper + 273.15) - 64.7013
            err = peval(k_abn, ph_out, ph_adj) - hr
            return err

        def peval(k_abn, ph_out, ph_adj):
            temp = k_abn[
                0] * np.power(10, -ph_out) + k_abn[1] + k_abn[2] * np.power(10, (-ph_adj + ph_out))
            return temp

        exams_obj.k_abn = list(
            leastsq(
                residuals,
                k_abn,
                args=(
    ...
    ##########################################################################
    # AMAZON KEY, store output files. You might have to write your own impo
    ##########################################################################
