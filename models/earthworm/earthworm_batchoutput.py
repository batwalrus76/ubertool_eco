...
def html_table(row_inp_all):
    while True:
        ...
        if row_inp_temp_all is None:
            ...
            earth = earthworm_model.earthworm(
                True,
                True,
                Kow_temp,
                L_temp,
            ...
            html_temp = earthworm_tables.table_all_batch(
                earthworm_tables.pvuheadings,
                earthworm_tables.sumheadings,
                earthworm_tables.tmpl,
                earth)
...
def loop_html(thefile):
    ...
    header = reader.next()
    # logger.info(header)
    # Create a job queue and add each row of batch temeplate file as a task
    # into it
    i = 1
    for row in reader:
        job_q.put([row, i])
    ...
    sum_html = earthworm_tables.table_all_sum(
        earthworm_tables.sumheadings,
        earthworm_tables.tmpl,
        Kow,
        L,
