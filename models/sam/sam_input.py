...
def samInputPage(request, model='', header='', formData=None):
    ...
    html = html + render_to_string(
        '04uberinput_tabbed_nav.html', {
            'nav_dict': {
                'class_name': [
                    'Chemical Fate', 'Application', 'Simulation', 'Output Options'], 'tab_label': [
    ...
    html = html + \
        """</table><table class="input_table tab tab_Application" style="display:none">"""
    html = html + str(sam_parameters.SamInp_app(formData))
    html = html + str(sam_parameters.SamInp_app_refine(formData))
    html = html + \
        """</table><table class="input_table tab tab_Simulation" style="display:none">"""
