...
def signed_area(coords):
    ...
    xs.append(xs[1])
    ys.append(ys[1])
    return sum(xs[i] * (ys[i + 1] - ys[i - 1])
               for i in range(1, len(coords))) / 2.0
        ...
        if self.shapeType in [POINT, POINTM, POINTZ]:
            ...
            if len(self.parts) == 1:
                ...
                for part in self.parts:
                    ...
                    coordinates.append(
                        tuple([tuple(p) for p in self.points[part:]]))
                return {
...
class Reader:
    ...
    def __shape(self):
        ...
        if nPoints:
            record.points = [
                _Array(
                    ...
                    unpack(
                        "<2d",
                        f.read(16))) for p in range(nPoints)]
        ...
        if shapeType in (
            ...
            # Measure values less than -10e38 are nodata values according to
            # the spec
            record.m = []
            for m in _Array(
                'd',
    ...
    def iterShapeRecords(self, numpyspeed=False):
        ...
        return (
            _ShapeRecord(
                shape=rec[0],
                record=rec[1]) for rec in itertools.izip(
                self.iterShapes(numpyspeed),
                self.iterRecords()))
...
class Writer:
    ...
    def __dbfRecords(self):
        ...
        for record in self.records:
            ...
            for (
                    fieldName, fieldType, size, dec), value in zip(
                    self.fields, record):
