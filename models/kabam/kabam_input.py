    ...
    html = html + render_to_string('04uberinput_tabbed_nav.html',
                                   {'nav_dict': {'class_name': ['Chemical',
                                                 ...
                                                 'tab_label': ['Chemical',
                                                               ...
                                                               'Constants']}})
    html = html + \
        """<br><table class="input_table tab tab_Chemical" border="0">"""
    html = html + str(kabam_parameters.KabamInp_chem(formData))
    html = html + \
        """</table><table class="input_table tab tab_Avian" border="0" style="display:none">"""
    html = html + str(kabam_parameters.KabamInp_bird(formData))
    html = html + \
        """</table><table class="input_table tab tab_Mammal" border="0" style="display:none">"""
    html = html + str(kabam_parameters.KabamInp_mammal(formData))
    html = html + \
        """</table><table class="input_table tab tab_LargeFish" border="0" style="display:none">"""
    html = html + str(kabam_parameters.KabamInp_lfish(formData))
    html = html + \
        """</table><table class="input_table tab tab_MediumFish" border="0" style="display:none">"""
    html = html + str(kabam_parameters.KabamInp_mfish(formData))
    html = html + \
        """</table><table class="input_table tab tab_SmallFish" border="0" style="display:none">"""
    html = html + str(kabam_parameters.KabamInp_sfish(formData))
    html = html + \
        """</table><table class="input_table tab tab_Filterfeeders" border="0" style="display:none">"""
    html = html + str(kabam_parameters.KabamInp_ff(formData))
    html = html + \
        """</table><table class="input_table tab tab_Invertebrates" border="0" style="display:none">"""
    html = html + str(kabam_parameters.KabamInp_invert(formData))
    html = html + \
        """</table><table class="input_table tab tab_Zooplankton" border="0" style="display:none">"""
    html = html + str(kabam_parameters.KabamInp_zoo(formData))
    html = html + \
        """</table><table class="input_table tab tab_Phytoplankton" border="0" style="display:none">"""
    html = html + str(kabam_parameters.KabamInp_phyto(formData))
    html = html + \
        """</table><table class="input_table tab tab_Sediment" border="0" style="display:none">"""
    html = html + str(kabam_parameters.KabamInp_sed(formData))
    html = html + \
        """</table><table class="input_table tab tab_Constants" border="0" style="display:none">"""
