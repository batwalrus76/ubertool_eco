...
def gett1data(pfam_obj):
    data = {
        "Parameter": [
            mark_safe('Water Column Half life @%s &#8451;') %
            pfam_obj.wat_t,
            mark_safe('Benthic Compartment Half Life @%s &#8451;') %
            pfam_obj.ben_t,
            mark_safe('Unflooded Soil Half Life @%s &#8451;') %
            pfam_obj.unf_t,
            'Aqueous Near-Surface Photolysis Half Life @%s Degrees Latitude' %
            pfam_obj.aqu_t,
...
def table_all(pfam_obj):
    ...
    html_all = table1_out + table2_out + table3_out + table4_out + \
        table5_out + table6_out + table7_out + table8_out + table9_out
    return html_all
...
def table_8(pfam_obj):
    ...
    <div class="out_">
      ...
      """ % (pfam_obj.final_res[1][0],
             pfam_obj.x_date1,
             pfam_obj.x_re_v_f,
             pfam_obj.x_re_c_f,
             pfam_obj.x_date2,
             pfam_obj.x_water,
             pfam_obj.x_water_level,
             pfam_obj.x_ben_tot,
             pfam_obj.x_ben_por)
    return html
