...
def getheaderpvu_2():
    headings = [
        "Application",
        "Days relevant to the application",
        "Application method",
        "Application rate",
        "Unit",
...
def gett1data(przm_exams_obj):
    data = {
        ...
        "Value": [
            '%s' %
            przm_exams_obj.chem_name,
            '%s' %
            przm_exams_obj.scenarios,
            '%s' %
            przm_exams_obj.met_o,
        ],
    }
    return data
...
def table_all(przm_exams_obj):
    ...
    html_all = table1_out + table2_out + table3_out + \
        table4_out + table5_out + table6_out + "</div></div>"
...
def table_1(przm_exams_obj):
    ...
    html = html + \
        tmpl.render(Context(dict(data=t1rows, headings=pvuheadings_1)))
    html = html + """
                </div>
        """
    return html
...
def table_2(przm_exams_obj):
    ...
    html = html + \
        tmpl.render(Context(dict(data=t2rows, headings=pvuheadings_2)))
    html = html + """
                </div>
        """
    return html
...
def table_4(przm_exams_obj):
    ...
    t4data = dict(
        [(k, [t4data_ind[k][0] for t4data_ind in t4data_all]) for k in t4data_temp])
...
def table_5(przm_exams_obj):
    html = """
                ...
                </table><br></div>""" % (przm_exams_obj.link,
                                         przm_exams_obj.x_pre_irr,
                                         przm_exams_obj.x_leachate,
                                         przm_exams_obj.x_et,
                                         przm_exams_obj.x_runoff,
                                         przm_exams_obj.Lim_inst,
                                         przm_exams_obj.Lim_24h,
                                         przm_exams_obj.Lim_96h,
                                         przm_exams_obj.Lim_21d,
                                         przm_exams_obj.Lim_60d,
                                         przm_exams_obj.Lim_90d,
                                         przm_exams_obj.Lim_y,
                                         przm_exams_obj.Ben_inst,
                                         przm_exams_obj.Ben_24h,
                                         przm_exams_obj.Ben_96h,
                                         przm_exams_obj.Ben_21d,
                                         przm_exams_obj.Ben_60d,
                                         przm_exams_obj.Ben_90d,
                                         przm_exams_obj.Ben_y)
    return html
