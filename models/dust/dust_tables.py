...
def gett1data(dust_obj):
    data = {
        "Parameter": [
            ...
            'Fraction of Pesticide Assumed at the Surface',
            'Dislodgeable Foliar Residue',
        ],
        "Value": [
            dust_obj.chemical_name,
            dust_obj.label_epa_reg_no,
            dust_obj.ar_lb,
            dust_obj.frac_pest_surface,
            dust_obj.dislodge_fol_res,
        ],
        "Units": [
            '',
            '',
...
def gett2data(dust_obj):
    data = {
        "Parameter": [
            'Bird Acute Oral Study (OCSPP 850.2100) MRID#',
            'Additional Comments About the Study (if any)',
            mark_safe('Lowest Bird Acute Oral LD<sub>50</sub> &asymp; Amphibian Dermal LD<sub>50</sub>'),
            'Tested Bird Body Weight',
            'Mineau Scaling Factor for Birds',
            mark_safe('Mammal Acute Dermal (OCSPP 870.1200) MRID#'),
            'Additional Comments About Study (if any)',
            mark_safe('Mammal Acute Dermal LD<sub>50</sub>'),
            'Avian Dermal type',
            mark_safe('Mammal Acute Oral LD<sub>50</sub>'),
            'Tested Mammal Body Weight',
        ],
        "Value": [
            dust_obj.bird_acute_oral_study,
            dust_obj.bird_study_add_comm,
            dust_obj.low_bird_acute_ld50,
            dust_obj.test_bird_bw,
            dust_obj.mineau_scaling_factor,
            dust_obj.mamm_acute_derm_study,
            dust_obj.mamm_study_add_comm,
            dust_obj.mam_acute_derm_ld50,
            dust_obj.mam_acute_oral_ld50,
            dust_obj.test_mam_bw,
        ],
        "Units": [
            '',
            '',
...
def gettsumdata(
    ...
    data = {
        ...
        "Mean": [
            ...
            numpy.mean(ar_lb),
            '%.2e' %
            numpy.mean(frac_pest_surface),
            '%.2e' %
            numpy.mean(dislodge_fol_res),
            '%.2e' %
            numpy.mean(low_bird_acute_ld50),
            '%.2e' %
            numpy.mean(test_bird_bw),
            '%.2e' %
            numpy.mean(mineau_scaling_factor),
            '%.2e' %
            numpy.mean(mam_acute_derm_ld50),
            '%.2e' %
            numpy.mean(test_mam_bw),
            '%.2e' %
            numpy.mean(mam_acute_oral_ld50),
        ],
        "Std": [
            '%.2e' %
            numpy.std(ar_lb),
            '%.2e' %
            numpy.std(frac_pest_surface),
            '%.2e' %
            numpy.std(dislodge_fol_res),
            '%.2e' %
            numpy.std(low_bird_acute_ld50),
            '%.2e' %
            numpy.std(test_bird_bw),
            '%.2e' %
            numpy.std(mineau_scaling_factor),
            '%.2e' %
            numpy.std(mam_acute_derm_ld50),
            '%.2e' %
            numpy.std(test_mam_bw),
            '%.2e' %
            numpy.std(mam_acute_oral_ld50),
        ],
        "Min": [
            '%.2e' %
            numpy.min(ar_lb),
            '%.2e' %
            numpy.min(frac_pest_surface),
            '%.2e' %
            numpy.min(dislodge_fol_res),
            '%.2e' %
            numpy.min(low_bird_acute_ld50),
            '%.2e' %
            numpy.min(test_bird_bw),
            '%.2e' %
            numpy.min(mineau_scaling_factor),
            '%.2e' %
            numpy.min(mam_acute_derm_ld50),
            '%.2e' %
            numpy.min(test_mam_bw),
            '%.2e' %
            numpy.min(mam_acute_oral_ld50),
        ],
        "Max": [
            '%.2e' %
            numpy.max(ar_lb),
            '%.2e' %
            numpy.max(frac_pest_surface),
            '%.2e' %
            numpy.max(dislodge_fol_res),
            '%.2e' %
            numpy.max(low_bird_acute_ld50),
            '%.2e' %
            numpy.max(test_bird_bw),
            '%.2e' %
            numpy.max(mineau_scaling_factor),
            '%.2e' %
            numpy.max(mam_acute_derm_ld50),
            '%.2e' %
            numpy.max(test_mam_bw),
            '%.2e' %
            numpy.max(mam_acute_oral_ld50),
        ],
        "Unit": [
            'lbs a.i./A',
            '',
...
def gettsumdata_out(
    ...
    data = {
        ...
        "Mean": ['%.2e' % numpy.mean(gran_bird_ex_derm_dose_out), '%.2e' % numpy.mean(gran_repamp_ex_derm_dose_out), '%.2e' % numpy.mean(gran_mam_ex_derm_dose_out),
                 '%.2e' % numpy.mean(fol_bird_ex_derm_dose_out), '%.2e' % numpy.mean(
                     fol_repamp_ex_derm_dose_out), '%.2e' % numpy.mean(fol_mam_ex_derm_dose_out),
                 '%.2e' % numpy.mean(bgs_bird_ex_derm_dose_out), '%.2e' % numpy.mean(
                     bgs_repamp_ex_derm_dose_out), '%.2e' % numpy.mean(bgs_mam_ex_derm_dose_out),
                 '%.2e' % numpy.mean(ratio_gran_bird_out), '%.2e' % numpy.mean(
                     ratio_gran_rep_out), '%.2e' % numpy.mean(ratio_gran_amp_out),
                 '%.2e' % numpy.mean(ratio_gran_mam_out), '%.2e' % numpy.mean(
                     ratio_fol_bird_out), '%.2e' % numpy.mean(ratio_fol_rep_out),
                 '%.2e' % numpy.mean(ratio_fol_amp_out), '%.2e' % numpy.mean(
                     ratio_fol_mam_out), '%.2e' % numpy.mean(ratio_bgs_bird_out),
        ...
        "Std": ['%.2e' % numpy.std(gran_bird_ex_derm_dose_out), '%.2e' % numpy.std(gran_repamp_ex_derm_dose_out), '%.2e' % numpy.std(gran_mam_ex_derm_dose_out),
                '%.2e' % numpy.std(fol_bird_ex_derm_dose_out), '%.2e' % numpy.std(
                    fol_repamp_ex_derm_dose_out), '%.2e' % numpy.std(fol_mam_ex_derm_dose_out),
                '%.2e' % numpy.std(bgs_bird_ex_derm_dose_out), '%.2e' % numpy.std(
                    bgs_repamp_ex_derm_dose_out), '%.2e' % numpy.std(bgs_mam_ex_derm_dose_out),
                '%.2e' % numpy.std(ratio_gran_bird_out), '%.2e' % numpy.std(
                    ratio_gran_rep_out), '%.2e' % numpy.std(ratio_gran_amp_out),
                '%.2e' % numpy.std(ratio_gran_mam_out), '%.2e' % numpy.std(
                    ratio_fol_bird_out), '%.2e' % numpy.std(ratio_fol_rep_out),
                '%.2e' % numpy.std(ratio_fol_amp_out), '%.2e' % numpy.std(
                    ratio_fol_mam_out), '%.2e' % numpy.std(ratio_bgs_bird_out),
        ...
        "Min": ['%.2e' % numpy.min(gran_bird_ex_derm_dose_out), '%.2e' % numpy.min(gran_repamp_ex_derm_dose_out), '%.2e' % numpy.min(gran_mam_ex_derm_dose_out),
                '%.2e' % numpy.min(fol_bird_ex_derm_dose_out), '%.2e' % numpy.min(
                    fol_repamp_ex_derm_dose_out), '%.2e' % numpy.min(fol_mam_ex_derm_dose_out),
                '%.2e' % numpy.min(bgs_bird_ex_derm_dose_out), '%.2e' % numpy.min(
                    bgs_repamp_ex_derm_dose_out), '%.2e' % numpy.min(bgs_mam_ex_derm_dose_out),
                '%.2e' % numpy.min(ratio_gran_bird_out), '%.2e' % numpy.min(
                    ratio_gran_rep_out), '%.2e' % numpy.min(ratio_gran_amp_out),
                '%.2e' % numpy.min(ratio_gran_mam_out), '%.2e' % numpy.min(
                    ratio_fol_bird_out), '%.2e' % numpy.min(ratio_fol_rep_out),
                '%.2e' % numpy.min(ratio_fol_amp_out), '%.2e' % numpy.min(
                    ratio_fol_mam_out), '%.2e' % numpy.min(ratio_bgs_bird_out),
        ...
        "Max": ['%.2e' % numpy.max(gran_bird_ex_derm_dose_out), '%.2e' % numpy.max(gran_repamp_ex_derm_dose_out), '%.2e' % numpy.max(gran_mam_ex_derm_dose_out),
                '%.2e' % numpy.max(fol_bird_ex_derm_dose_out), '%.2e' % numpy.max(
                    fol_repamp_ex_derm_dose_out), '%.2e' % numpy.max(fol_mam_ex_derm_dose_out),
                '%.2e' % numpy.max(bgs_bird_ex_derm_dose_out), '%.2e' % numpy.max(
                    bgs_repamp_ex_derm_dose_out), '%.2e' % numpy.max(bgs_mam_ex_derm_dose_out),
                '%.2e' % numpy.max(ratio_gran_bird_out), '%.2e' % numpy.max(
                    ratio_gran_rep_out), '%.2e' % numpy.max(ratio_gran_amp_out),
                '%.2e' % numpy.max(ratio_gran_mam_out), '%.2e' % numpy.max(
                    ratio_fol_bird_out), '%.2e' % numpy.max(ratio_fol_rep_out),
                '%.2e' % numpy.max(ratio_fol_amp_out), '%.2e' % numpy.max(
                    ratio_fol_mam_out), '%.2e' % numpy.max(ratio_bgs_bird_out),
...
def table_all(dust_obj):
    ...
    # return html_all, table3_out, table4_out, table5_out, table6_out,
    # table7_out, table8_out
    return html_all
...
def table_sum_output(
    ...
    # table sum_input
    tsumoutputdata = gettsumdata_out(
        gran_bird_ex_derm_dose_out,
        gran_repamp_ex_derm_dose_out,
        gran_mam_ex_derm_dose_out,
        fol_bird_ex_derm_dose_out,
    ...
    html = html + \
        tmpl.render(Context(dict(data=tsumoutputrows, headings=sumheadings)))
    html = html + """
                </div>
        </div>
        """
...
def table_3(dust_obj):
    ...
    return {
        'html': html,
        'granbirdderm': granbirdderm,
        'granherpderm': granherpderm,
        'granmammderm': granmammderm}
...
def table_4(dust_obj):
    ...
    return {
        'html': html,
        'folbirdderm': folbirdderm,
        'folherpderm': folherpderm,
        'folmammderm': folmammderm}
...
def table_5(dust_obj):
    ...
    return {
        'html': html,
        'barebirdderm': barebirdderm,
        'bareherpderm': bareherpderm,
        'baremammderm': baremammderm}
...
def table_6(dust_obj):
    ...
    return {
        'html': html,
        'granbirdrisk': granbirdrisk,
        'granreprisk': granreprisk,
        'granamphibrisk': granamphibrisk,
        'granmammrisk': granmammrisk}
...
def table_8(dust_obj):
    ...
    return {
        'html': html,
        'barebirdrisk': barebirdrisk,
        'barereprisk': barereprisk,
        'bareamphibrisk': bareamphibrisk,
        'baremammrisk': baremammrisk}
