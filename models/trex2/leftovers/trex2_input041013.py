...
class trexInputPage(webapp.RequestHandler):
    ...
    def get(self):
        ...
        html = html + \
            template.render(templatepath + '02uberintroblock_wmodellinks.html', {'model': 'trex2', 'page': 'input'})
        html = html + \
        ...
        html = html + \
            """</table><table class="input_table tab tab_Animal" border="0">"""
        html = html + str(trexdb2.trexAnimal())
        html = html + template.render(templatepath + 'trex2-jquery.html', {})
        html = html + \
            template.render(templatepath + '04uberinput_end.html', {'sub_title': 'Submit'})
        html = html + \
