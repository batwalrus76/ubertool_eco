    ...
    if n_a == 1:
        ...
        for i in range(0, 365):
            if i == 0:  # first day of application
                ...
                dayt = dayt + 1
            # next application day
            elif i == day_out[dayt] and n_a_temp <= n_a:
                n_a_temp = n_a_temp + 1
        ...
        #     C_temp[i]=C_t(C_temp[i-1], h_l)
            # not an application day just degredation of existing concentration
            else:
                C_temp[i] = C_t(C_temp[i - 1], h_l)
        return (max(C_temp))
...
def LD50_rg_bird(
    ...
    if Application_type == 'Row/Band/In-furrow-Granular':
        ...
        expo_rg_bird = (max(a_r) * a_i * 453590.0) / \
            (n_r * (43560.0 ** 0.5) * b_w) * (1 - p_i)
...
def LD50_rg_mamm(
    ...
    if Application_type == 'Row/Band/In-furrow-Granular':
       # a_r = max(rate_out)
        at_mamm = at_mamm(ld50_mamm, aw_mamm, tw_mamm)
        n_r = (43560 ** 0.5) / (r_s)
        expo_rg_mamm = (max(a_r) * a_i * 453590) / \
            (n_r * (43560 ** 0.5) * b_w) * (1 - p_i)
...
def sa_bird_1(
    ...
    m_s_a_r = ((a_r_p * a_i) / 128) * den * 10000
...
def sa_bird_2(
    ...
    m_a_r = ((((a_r_p * a_i) / 128) * den) * m_s_r_p) / 100
...
class TRexOutputPage(webapp.RequestHandler):
    ...
    def post(self):
     ...
     #   a_r_l=float(a_r_l)
        seed_treatment_formulation_name = form.getvalue(
...
#        bird_type = form.getvalue('Bird_type')
        Species_of_the_tested_bird = form.getvalue(
...
#           mf_w_bird=0.1
        ...
        html = template.render(
            templatepath + '01uberheader.html', {'title': 'Ubertool'})
        html = html + \
            template.render(templatepath + '02uberintroblock_wmodellinks.html', {'model': 'trex2', 'page': 'output'})
        html = html + \
            template.render(templatepath + '03ubertext_links_left.html', {})
        ...
        html = html + \
            template.render(templatepath + '04uberoutput_end.html', {'sub_title': ''})
        html = html + \
            template.render(templatepath + '05ubertext_links_right.html', {})
        html = html + \
            template.render(templatepath + '06uberfooter.html', {'links': ''})
