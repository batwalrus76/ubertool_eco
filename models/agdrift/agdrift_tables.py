...
def gett1data(agdrift_obj):
    data = {
        ...
        "Value": [
            agdrift_obj.application_method,
            agdrift_obj.orchard_type,
            agdrift_obj.drop_size,
            agdrift_obj.ecosystem_type,
        ],
    }
    return data
...
# def
# gettsumdata_out(init_avg_dep_foa,avg_depo_lbac,avg_depo_gha,deposition_ngL,deposition_mgcm,nasae,y,x,express_y):
...
def gettsumdata_out(
    ...
    data = {
        ...
        "Mean": [
            '%.2e' %
            numpy.mean(init_avg_dep_foa_out),
            '%.2e' %
            numpy.mean(avg_depo_lbac_out),
            '%.2e' %
            numpy.mean(avg_depo_gha_out),
            '%.2e' %
            numpy.mean(deposition_ngL_out),
            '%.2e' %
            numpy.mean(deposition_mgcm_out),
            '%.2e' %
            numpy.mean(nasae_out),
            '%.2e' %
            numpy.mean(y_out),
            '%.2e' %
            numpy.mean(x_out),
            '%.2e' %
            numpy.mean(express_y_out),
        ],
        "Std": [
            '%.2e' %
            numpy.std(init_avg_dep_foa_out),
            '%.2e' %
            numpy.std(avg_depo_lbac_out),
            '%.2e' %
            numpy.std(avg_depo_gha_out),
            '%.2e' %
            numpy.std(deposition_ngL_out),
            '%.2e' %
            numpy.std(deposition_mgcm_out),
            '%.2e' %
            numpy.std(nasae_out),
            '%.2e' %
            numpy.std(y_out),
            ...
            numpy.std(x_out),
            '%.2e' %
            numpy.std(express_y_out),
        ],
        "Min": [
            '%.2e' %
            numpy.min(init_avg_dep_foa_out),
            '%.2e' %
            numpy.min(avg_depo_lbac_out),
            '%.2e' %
            numpy.min(avg_depo_gha_out),
            '%.2e' %
            numpy.min(deposition_ngL_out),
            '%.2e' %
            numpy.min(deposition_mgcm_out),
            '%.2e' %
            numpy.min(nasae_out),
            '%.2e' %
            numpy.min(y_out),
            ...
            numpy.min(x_out),
            '%.2e' %
            numpy.min(express_y_out),
        ],
        "Max": [
            '%.2e' %
            numpy.max(init_avg_dep_foa_out),
            '%.2e' %
            numpy.max(avg_depo_lbac_out),
            '%.2e' %
            numpy.max(avg_depo_gha_out),
            '%.2e' %
            numpy.max(deposition_ngL_out),
            '%.2e' %
            numpy.max(deposition_mgcm_out),
            '%.2e' %
            numpy.max(nasae_out),
            '%.2e' %
            numpy.max(y_out),
...
def table_all(agdrift_obj):
    ...
    html_all = html_all + \
        render_to_string('agdrift-output-jqplot.html', {'chart_num': agdrift_obj.loop_indx})
    return html_all
