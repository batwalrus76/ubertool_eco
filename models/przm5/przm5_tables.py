...
def gett1data(przm5_obj):
    data = {
        ...
        "Value": [
            '%s' %
            przm5_obj.koc_check_text,
            '%s' %
            przm5_obj.Koc[0],
            '%s' %
            przm5_obj.soilHalfLifeBox[0],
            '%s' %
            przm5_obj.soilTempBox1,
            '%s' %
            przm5_obj.foliarHalfLifeBox[0],
            '%s' %
            przm5_obj.deg_check],
        "Units": [
            '',
...
def gett1data_b(przm5_obj):
    data = {
        "Parameter": [
            ...
            'Soil Reference Temp',
            'Foliar Halflife',
            'Molar Conversion Factors (Soil)',
            'Molar Conversion Factors (Foliar)',
        ],
        "Value": [
            '%s' %
            przm5_obj.Koc[1],
            '%s' %
            przm5_obj.soilHalfLifeBox[1],
            25,
            '%s' %
            przm5_obj.foliarHalfLifeBox[1],
            '%s' %
            przm5_obj.convertSoil1,
            '%s' %
            przm5_obj.convert_Foliar1,
        ],
        "Units": [
            'mL/g',
            'day',
...
def gett1data_c(przm5_obj):
    data = {
        ...
        "Value": [
            ...
            przm5_obj.Koc[2],
            '%s' %
            przm5_obj.soilHalfLifeBox[2],
            25,
            '%s' %
            przm5_obj.foliarHalfLifeBox[2],
            '%s' %
            przm5_obj.deg2_source_text,
            '%s' %
            przm5_obj.convertSoil2,
            '%s' %
            przm5_obj.convert_Foliar2,
        ],
        "Units": [
            'mL/g',
            'day',
...
def gett2data(przm5_obj):
    data = {
        "Parameter": [
            'Choose Way of Entering Application Dates',
    ...
    if przm5_obj.tempflag == 0:
        data = {
            ...
            "Value": [
                ...
                przm5_obj.dvf_file,
                '%s' %
                przm5_obj.Emerge_text,
                '%s' %
                przm5_obj.Mature_text,
                '%s' %
                przm5_obj.Harvest_text,
                '%s' %
                przm5_obj.pfac,
                '%s' %
                przm5_obj.snowmelt,
                ...
                przm5_obj.rootDepth,
                '%s' %
                przm5_obj.canopyCover,
                '%s' %
                przm5_obj.canopyHeight,
                '%s' %
                przm5_obj.canopyHoldup,
                '%s' %
                przm5_obj.irflag_text,
                '%s' %
                przm5_obj.fleach,
                '%s' %
                przm5_obj.depletion,
                ...
                przm5_obj.rateIrrig,
                '%s' %
                przm5_obj.PestDispHarvest_text,
                '%s' %
                przm5_obj.noh,
                '%s' %
                'No',
...
def gett4data(przm5_obj):
    data = {
        ...
        "Value": [
            ...
            przm5_obj.rDepthBox,
            '%s' %
            przm5_obj.rDeclineBox,
            '%s' %
            przm5_obj.rBypassBox,
            '%s' %
            przm5_obj.eDepthBox,
            '%s' %
            przm5_obj.eDeclineBox,
        ],
        "Units": [
            '',
            '',
...
def timestamp(przm5_obj):
    ...
    st = datetime.datetime.strptime(
        przm5_obj.jid,
        '%Y%m%d%H%M%S%f').strftime('%A %Y-%m-%d %H:%M:%S')
    html = """
    <div class="out_">
        <b>PRZM 5<br>
    """
...
def table_all(przm5_obj):
    ...
    html_all = table1_out_all + table2_out + table3_out + table4_out + \
        table5_out + table6_out + table7_out + table8_out + html_plot
    return html_all
...
def table_1_b(przm5_obj):
    ...
    html = html + \
        tmpl.render(Context(dict(data=t1rows_b, headings=pvuheadings)))
    html = html + """
            </div>
    """
    return html
...
def table_1_c(przm5_obj):
    ...
    html = html + \
        tmpl.render(Context(dict(data=t1rows_c, headings=pvuheadings)))
    html = html + """
            </div>
    """
    return html
...
def table_2(przm5_obj):
    ...
    html = html + \
        tmpl.render(Context(dict(data=t2rows, headings=pvuheadings2)))
    html = html + """
            </div>
    """
    return html
...
def table_2_b(przm5_obj):
    ...
    t2_b_data = dict(
        [(k, [t2data_ind[k][0] for t2data_ind in t2_b_data_all]) for k in t2_d_data_temp])
    t2_b_rows = gethtmlrowsfromcols(t2_b_data, table_2b_headings[0])
    html = html + \
        tmpl.render(Context(dict(data=t2_b_rows, headings=table_2b_headings[1])))
    html = html + """
                </div>
        """
    return html
...
def table_3_b(przm5_obj):
    ...
    t3_b_data = dict(
        [(k, [t3data_ind[k][0] for t3data_ind in t3_b_data_all]) for k in t3_b_data_temp])
    t3_b_rows = gethtmlrowsfromcols(t3_b_data, table_3b_headings[0])
    html = html + \
        tmpl.render(Context(dict(data=t3_b_rows, headings=table_3b_headings[1])))
    html = html + """
                </div>
        """
    return html
...
def table_4_b(przm5_obj):
    ...
    t4_b_data = dict(
        [(k, [t4data_ind[k][0] for t4data_ind in t4_b_data_all]) for k in t4_b_data_temp])
    t4_b_rows = gethtmlrowsfromcols(t4_b_data, table_4b_headings[0])
    html = html + \
        tmpl.render(Context(dict(data=t4_b_rows, headings=table_4b_headings[1])))
    html = html + """
                </div>
        """
    return html
