...
def przm5InputPage(request, model='', header='', formData=None):
    ...
    html = html + \
        render_to_string('04uberinput_jquery.html', {'model': "swcprzm5vvwm"})
    html = html + render_to_string('04uberinput_start_tabbed.html', {
        'model': model,
        'model_attributes': header + ' Inputs'})

    html = html + render_to_string(
        '04uberinput_tabbed_nav.html', {
            'nav_dict': {
                'class_name': [
                    'Chemical', 'Applications', 'CropLand', "Runoff", "WaterBody"], 'tab_label': [
                    'Chemical', 'Applications', 'Crop/Land', "Runoff", "Water Body"]}})
    ...
    html = html + \
        """</table><table class="input_table tab tab_Chemical_MCF1" style="display:none">
    ...
    html = html + \
        """</table><table class="input_table tab tab_Chemical_MCF2" style="display:none">
                        <tr><th colspan="2">Molar Conversion Factors (Degradate 2)</th></tr>
                        """
    html = html + str(przm5_parameters.przm5Inp_mcf2(formData))

    html = html + \
        """</table><table class="input_table tab tab_Applications" style="display:none">"""
    ...
    html = html + """
                    ...
                    """
    # Next line will be replaced with "vvwm_weatherfile.html" template when
    # selecting of Weatherfile (*.dvf) is enabled
    html = html + \
        """</table><table class="input_table tab tab_CropLand" style="display:none">"""
    ...
    html = html + \
        """</table><table class="input_table tab tab_Runoff" style="display:none">"""
    html = html + str(przm5_parameters.przm5Inp_runoff(formData))
    html = html + \
        """</table><table class="input_table tab tab_Runoff tab_nott" style="display:none">
    ...
    html = html + \
        """</table><table class="input_table tab tab_WaterBody" style="display:none">"""
