...
class PRZMInp(forms.Form):
    ...
    application_efficiency = forms.ChoiceField(
        required=True,
        choices=EFFICIENCY,
        label='Application efficiency (0.95 for aerial; 0.99 for ground,airblast)')
    spray_drift = forms.FloatField(
        required=True,
        label='Spray Drift (fraction)')
    molecular_weight = forms.FloatField(
        required=True,
        label='Molecular weight (g/mol)')
    henrys_law_constant = forms.FloatField(
        required=True,
        label="Henry's Law Constant (atm-m^3/mol)")
    ...
    anaerobic_aquatic_metabolism = forms.FloatField(
        required=True,
        label='Anaerobic aquatic metabolism (half-life, days)')
    aerobic_soil_metabolism = forms.FloatField(
        required=True,
        label='Aerobic soil metabolism (half-life, days)')
    hydrolysis_ph5 = forms.FloatField(
        required=True,
        label='Hydrolysis: pH=5/acidic (half-life, days)')
    hydrolysis_ph7 = forms.FloatField(
        required=True,
        label='Hydrolysis: pH=7/neutral (half-life, days)')
    hydrolysis_ph9 = forms.FloatField(
        required=True,
        label='Hydrolysis: pH=9/alkaline (half-life, days)')
    foliar_extraction = forms.FloatField(
        required=True,
        label='Foliar extraction, default=0.5 (cm^-1)')
    foliar_decay_rate = forms.FloatField(
        required=True,
        label='Foliar Decay Rate (day^-1)')
