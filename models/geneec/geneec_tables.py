...
def gett1dataAerial(geneec_obj):
    ...
    data = {
        "Parameter": [
            ...
            'Wet in',
            'Aerial size dist',
            'Width of the no-spray zone',
            'Solubility',
            'Aerobic aquatic metabolism half-life',
            'Photolysis, aquatic half-life'],
        "Value": [
            geneec_obj.chem_name,
            geneec_obj.application_target_label,
            '%s' %
            geneec_obj.application_rate,
            '%s' %
            geneec_obj.number_of_applications,
            '%s' %
            geneec_obj.interval_between_applications,
            '%s' %
            geneec_obj.Koc,
            '%s' %
            geneec_obj.aerobic_soil_metabolism,
            '%s' %
            geneec_obj.wet_in,
            '%s' %
            aerial_dist,
            '%s' %
            geneec_obj.no_spray_drift,
            '%s' %
            geneec_obj.solubility,
            '%s' %
            geneec_obj.aerobic_aquatic_metabolism,
            '%s' %
            geneec_obj.photolysis_aquatic_half_life],
        "Units": [
            '',
            '',
            'lbs a.i./A',
...
def gett1dataAerial_qaqc(geneec_obj):
    ...
    data = {
        "Parameter": [
            ...
            'Photolysis, aquatic half-life'],
        "Value": [
            geneec_obj.chem_name_exp,
            geneec_obj.application_target,
            '%s' %
            geneec_obj.application_rate,
            '%s' %
            geneec_obj.number_of_applications,
...
def gett1dataGround(geneec_obj):
    ...
    data = {
        ...
        "Value": [
            ...
            geneec_obj.no_spray_drift,
            '%s' %
            geneec_obj.incorporation_depth,
            '%s' %
            geneec_obj.solubility,
            '%s' %
            geneec_obj.aerobic_aquatic_metabolism,
...
def gett2data(geneec_obj):
    data = {
        "Parameter": [
            'Peak GEEC',
            'Maximum 4-day Average GEEC',
            'Maximum 21-day Average GEEC',
            'Maximum 60-day Average GEEC',
            'Maximum 90-day Average GEEC'],
        "Value": [
            '%.1f' %
            geneec_obj.output_val[0],
            '%.1f' %
            geneec_obj.output_val[1],
            '%.1f' %
            geneec_obj.output_val[2],
            '%.1f' %
            geneec_obj.output_val[3],
            '%.1f' %
            geneec_obj.output_val[4]],
        "Units": [
            'ppb',
            'ppb',
            'ppb',
...
def gett2data_qaqc(geneec_obj):
    data = {
        ...
        "Expected Value": [
            '%.1f' %
            geneec_obj.GEEC_peak_exp,
            '%.1f' %
            geneec_obj.GEEC_4avg_exp,
            '%.1f' %
            geneec_obj.GEEC_21avg_exp,
            '%.1f' %
            geneec_obj.GEEC_60avg_exp,
            '%.1f' %
            geneec_obj.GEEC_90avg_exp],
        "Units": [
            'ppb',
            'ppb',
            'ppb',
...
def table_2_qaqc(geneec_obj):
    ...
    html = html + \
        tmpl.render(Context(dict(data=t2rows, headings=pvuheadingsqaqc)))
    html = html + """
                </div>
        </div>
        """
